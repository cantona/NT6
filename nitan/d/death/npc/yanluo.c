...
void create()
{
        ...
        set("chat_msg",({
                CYN "十殿閻羅深深的歎了口氣。\n" NOR,
                CYN "十殿閻羅歎道：玄武甲殼和朱雀羽毛還好點，可是青龍膽和白虎骨難弄啊……\n" NOR,
                CYN "十殿閻羅歎道：武甲、朱羽、龍膽還有虎骨，這可真難辦啊。\n" NOR,
                CYN "十殿閻羅歎道：期限就要到了，地藏老大要我煉製的鎖陽丹原料還沒備齊，這可如何是好啊。\n" NOR,
                CYN "十殿閻羅歎道：地藏老大的任務可真是要命啊。\n" NOR,
        ...
        set("inquiry", ([
                ...
                "地陰血脈" : "這東西可以在地府輪迴司裏尋找到。…嗯…要決是‘三三逢緣’。",
                "白虎"     : "那大蟲就棲息在寂滅司後面的黑森林裏，我派了好幾個手下去捉它，結果全都掛了。",
                "黑森林"   : "黑森林裏全是陰霧，除非有「指南車」，要不根本沒法辨認方向。",
                "指南車"   : "那是黃帝流傳下來的法寶之一，後來流落到了這裏，不過我沒見過。",
                "寂滅司"   : "寂滅司就在閻羅殿的東邊。",
                "輪迴司"   : "輪迴司就在閻羅殿的東邊。",
                "閻羅殿"   : "這個你也問我？你是笨死的啊？",
                "朱雀"     : "有人在陰陽塔頂見到過神獸朱雀，不過聽説那扁毛畜生不落無寶之地，要尋它身上必須要帶有寶物。",
                "寶物"     : "地府有個神石名曰「七色瓔珞」，倒是個不得多見的寶物。",
                "七色瓔珞" : "聽傳聞，這東西好像是落在了秦廣王的手裏。",
                "秦廣王"   : "他和我一樣，也是陰間十王之一。",
                "陰陽塔"   : "就是西邊的那座塔，是地府裏最高的建築，平時由馬面看守。",
                "馬面"     : "和看守奈河橋的牛頭一樣，是地府的守衞。",
                "玄武"     : "那臭烏龜整年躲在血池地獄裏吃死人肉，怎麼弄都不出來。",
...
int do_accept()
{
        ...
        if( query_temp("to_answer", me) )
        {
                ...
                message_vision( CYN "\n十殿閻羅「哈哈哈」大笑幾聲。\n"
                                NOR, me);
                message_vision( CYN "十殿閻羅對$N" CYN "説道：那老夫就"
                                "多謝謝你了。\n"NOR, me);
...
int give_gold(object me)
{
        message_vision(HIW "\n十殿閻羅隨手一揮，登時變出一大堆金子，遞"
                       "給$N" HIW "。\n"NOR, me);
        message_vision(CYN "十殿閻羅説道：你陽間的武器在這裏召喚不回"
                       "來，這裏的黃金足夠你購買武具。\n"NOR, me);
...
int accept_object(object me, object ob)
{
     ...
     && query("id", ob) != "xuanwu jiake" )
        ...
        if( !query("hell_quest/鎖陽丹", me) )
        {
                message_vision(CYN "十殿閻羅道：無功不受祿，這個你還是自己留着吧。\n"
                               NOR, me);
                        return 0;
        }
...
mixed ask_reborn()
{
        ...
        */
        ...
        {
                message_vision("$N對$n一拱手，道：你已經挑戰過六道守衞佛了，我送你找地藏王吧！\n",
                               this_object(), who);
        ...
        foreach( object user in all )
                if( userp(user) )
                        return notify_fail("你先把你身上揹着的人放下來再説。\n");
...
{
        command("say 這裏再怎麼説也是俠客行底層改過來的，要拜地府玩西遊記去。 ");
        return;
}
