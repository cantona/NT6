...
相比，這個廟又小又簡陋，但是據傳這裡的釋迦牟尼金身像乃當年鑒
真從天竺所帶來，無比靈異，因此這裡常年香火鼎盛。佛門講究一個
緣字，重視人的先天根骨，不少玩家為了讓自己擁有超常的根骨，經
常來這裡頂禮膜拜(mobai)。 新玩家也可以在這裡抽簽(chouqian)，
明確一下自己的性格，踏上漫漫江湖路。
LONG );
...
int do_chouqian(string arg)
{
        ...
        if( time()-query("last_chouqian_time", me)<1800 )
                return notify_fail("這麼頻繁的抽簽幹嘛？心不誠則簽不靈，過半個小時再來吧。\n");

        write(HIC "你虔誠的抽了一根簽，閉目祈禱了一陣，才睜開眼睛看簽上所書文字。\n" NOR, me);
