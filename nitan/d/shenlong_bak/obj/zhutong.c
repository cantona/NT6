...
void create()
{
        ...
        set("unit", "個");
        set("long", "這是一節青竹製成的竹筒，如果在裏面放上誘餌，可以用來捕捉毒蟲(entice)。\n");
        set("value", 100);
...
int do_entice(string arg)
{
    ...
    if (sizeof(all_inventory(ob1)) >= 2) 
        return notify_fail("竹筒裏面已經有東西了。\n");              
    message_vision("$N將竹筒的塞子拔掉，對着"+query("name", ob)+"輕輕的吹着口哨。\n",me);
    if(me->query_skill("poison",1) < 80)
    {
        ...
        message_vision("只見"+query("name", ob)+"在竹筒邊探頭探腦的聞了聞，忽然一閃，鑽進了$N的竹筒裏。\n",me);
...
//  ob1= this_object();
    ...
    if( !target
        ||      !me->is_fighting(target) )
                return notify_fail(query("name", ob)+"只能用於戰鬥中暗算對手。\n");
    if( query("family/family_name", me) != "神龍教"
         ||me->query_skill("poison",1) < 50)
    {
         tell_object(ob, HIG "你從顫抖着從懷裏掏出竹筒，但怎麼也沒有膽量打開它！\n" NOR );
         message_vision(HIG"只見$N從懷裏掏出一個竹筒，臉色發白，手腳顫抖不知道要幹什麼？\n",me);
    }
    tell_object(me,  "你悄悄從懷裏掏出一個竹筒，打開了塞子。\n" );
    message_vision("只見$N虛晃一招把手一揚，一道閃光射向"+query("name", target)+"的咽喉。\n",me);
    ...
    message_vision("只見"+query("name", ob)+"身子在空中一折，咬向"+query("name", target)+"的頭頂。\n",me);
        COMBAT_D->do_attack(ob,target,query_temp("weapon", ob),0);
    message_vision("只見"+query("name", ob)+"尾巴一彈，在空中一個迴轉竟繞到了"+query("name", target)+"的背後。\n",me);
        COMBAT_D->do_attack(ob,target,query_temp("weapon", ob),0);
