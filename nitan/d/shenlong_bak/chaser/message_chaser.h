        ...
        for(i = 0; i < sizeof(filenames); i++)
        {
                ...
                if( !userp(owner) && query("no_fight", owner) )
                {
                        ...
                        if( !userp(owner))
                        {
                                ...
                                {
                                        message("channel:snow",sprintf(HIM"【謠言】某人：聽說%s"+HIM+"好象落在%s的%s"+HIM+"手裡了！\n"NOR,
                                        ob->name(),
                                ...
                                message("channel:snow",sprintf(HIM"【謠言】某人：聽說%s"+HIM+"好象落在%s"+HIM+"手裡了！\n"NOR, 
                                ob->name(),
        ...
        if( query("no_fight", dest) )
        {
                ...
                else
                {
                        ...
                        if( !living(thief_ob) || thief_ob->is_fighting() )
                                return;
                        if( dest == room ) return;
                        message("vision", thief_ob->name() + "急急忙忙地離開了。\n", room, ({thief_ob}));
                }
                thief_ob->move(dest);
                message("vision", thief_ob->name() + "走了過來。\n", dest, ({thief_ob}));
                return;
        }
...
//                "ren woxing",
        ...
        if( !clonep(ob) || userp(ob) || query("race", ob) != "人類")return 0;
