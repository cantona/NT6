...
void create()
{
        ...
        set("age", 56);
        set("long", @LONG
這是一個老船夫。飽經風霜的臉上透出一絲狡猾。他經常出海，以前
...
int chu_hai()
{
        ...
        if( query("party/party_name", ob) == "神龍教" )
        {
                ...
                message_vision("年輕船夫和$N上了船，一聲「起錨」船就開走了，但"
                               "老船夫卻沒上船......\n", ob);
                ...
                message_vision("老船夫上上下下打量了$N一下：什麼！出海？你"
                               "不要命了？！\n老船夫又想了一下：不過...如"
                               "果給我一兩金子，或許可以考慮。\n", ob);
        set_temp("出海", 1, ob);
        return 1;
...
int accept_object(object who, object ob)
{
        ...
        {
                message_vision("老船夫笑著對$N說：無功不受祿，您"
                               "還是自己留著吧！\n", who);
                return 0;
        }
        if( query("money_id", ob) && ob->value() >= 10000 )
        {
                message_vision("老船夫對$N說：好！既然這位" +
                ...
                tell_room(myenv, "老船夫看船走了，掂著手裡的金子笑了：去"
                          "神龍島？送死的事情我可不幹。\n只可惜我那幾個"
                          "聽話的船夫，要被那幫人給吃掉了。嘿嘿，哈哈！\n");
