...
{
        set("short", "泥潭注冊室");
        set("long", "\n這裡是新玩家進行注冊的地方，請您使用指令 " HIC "register " NOR "加上你的完\n"
                    "整及真實" HIC " email " NOR "地址進行注冊。\n\n"
                    "    請務必認真注冊你的電子郵件地址，" HIR "電子郵件地址是保障你一切\n"
                    "權益的依據" NOR "，包括你密碼的丟失情況下，我們通過你注冊的信箱來確\n"
                    "認你的身份和歸還你的密碼。\n\n"
        ...
        set("item_desc", ([
        "sign":
"\t     [1;33m有關電子郵件注冊的幾點說明[0m
[37m－－－－－－－－－－－－－－－－－－－－－－－－－－[0m
首先請你仔細檢查注冊命令裡輸入的電子郵件地址是否正確！

這個遊戲的注冊過程是由機器自動完成的，在你還沒有離開
遊戲的時候注冊信就已經發出了。如果注冊信未能正確發出
會提示你從新輸入注冊命令，只有正確發出以後才會將注冊
玩家斷線。
...
int do_register(string arg)
{
        ...
        else
        {
                ...
                msg += "    如果很不巧的您是一個利欲熏心的人，這裡的官職系統一定可以\n" +
                       "滿足您擔當兵馬大將軍的欲望。\n";
                        ...
                        query("email", me),"感謝您光臨泥潭網絡遊戲",msg) )
                {
                        ...
                        tell_object(me, "發送注冊郵件失敗，請注冊你正確的郵件地址！\n");
                        return 1;
                }
                */
