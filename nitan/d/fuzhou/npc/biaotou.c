...
string * inroad_msg = ({
        ...
        "$N對$n説道：“『忍得一時之氣，可免百日之災。』上次在這吃過虧，這次往那邊閃吧。”\n",
        ...
        "$N滿臉堆笑，對$n説道：“『光棍不吃眼前虧，伸手不打笑臉人。』在下到那邊方便方便。”\n",
});
...
int do_baobiao(string arg)
{
        ...
        if( !query_temp("fuwei_ready", me) )
                return notify_fail(query("name", this_object())+"説：“去去去，去帳房交錢去！”\n");
        if( query("leader_name") != query("id", me) )
                return notify_fail(query("name", this_object())+"説：“你沒有負責保護此鏢的義務！”\n");
