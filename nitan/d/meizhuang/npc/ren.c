...
int ask_bijian()
{
        ...
        message_vision(CYN "$N" CYN "躬身説道：晚輩今日有幸拜見"
                       "任老前輩，還望多加指教。\n$n" CYN "笑道"
                       "：不用客氣，你來解我寂寞，可多謝你啦。"
                       "\n" NOR, me, ob );
...
int do_accept()
{
        ...
        if( query_temp("want_bijian", me) )
        {
                ...
                obj->move(me);
                say(CYN "\n任我行哈哈大笑説道：閣下果然武藝超羣，再來接我這路劍法！\n\n" NOR);

                message_vision(HIR "説完任我行一聲長嘯，木劍倏地劈出。$N" HIR "當即斜劍"
                               "刺出，逼得他收劍回擋。任\n我行連連呼喝，竟似發瘋一般。呼"
