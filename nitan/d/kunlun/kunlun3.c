...
int do_throw(string arg)
{
        ...
        if( query("betrayer", me) )
                return notify_fail("你與判師門派中的糾葛未了，無法使用昆侖鏡！\n");

        if( !arg || arg != "contract" )
                return notify_fail("你要用什麼擲向那冰壁？\n");

        message_vision( HIW "$N" HIW "將泛紫光的閻王契(Contract)擲向那冰壁，閻王契沒入其中、竟爾消失不見。\n"
                        HIW "只見本就平滑的大冰壁癒見光潔、而後漸漸透明，漸漸化作了水銀鏡般的模樣，\n"
                        HIW "原來這正是通往地獄的通道：" HIR "「昆侖鏡」" HIW "(enter mirror)。\n" NOR, me );
...
int do_enter(string arg)
{
        ...
        for( i=sizeof(inv)-1; i>=0; i-- ) {
                if( objectp(inv[i]) && inv[i]->is_character() ) {
                        tell_object(me, "你不可以將其他玩家或者NPC(包括寵物)背進昆侖鏡中。\n");
                        return 0;
                }
        }

        message_vision( HIW "$N" HIW "謹慎舉步，緩緩走入了昆侖鏡中。\n" NOR, me );
