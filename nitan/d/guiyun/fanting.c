...
int do_sit(string arg)
{
        ...
        message_vision("$N找了個空位座下，等着吃飯。\n", me);        
        local = localtime(time() * 60);
        if (local[2] < 6) {
                message_vision("僕役走過來對$N説：大半夜的您讓小的去哪裏弄吃的啊？\n", me);
        ...
        if( query_temp("marks/food_served", me)){
                message_vision("僕役走過來對$N説：吃飽了的就出去吧，別佔着地方！\n", me);
...
//        if (random(3)) 
                return notify_fail("僕役跑過來説：沒吃完的也別帶走啊，考慮一下別人嘛。\n");
