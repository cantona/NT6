...
void create()
{
        set("short", "觀雲台");
        set("long", @LONG
這裏是武當山天柱峯的觀雲台。放眼望去，只見雲海湧動，氣勢
磅礴。你心中微微一動，凝神靜思，感覺在這裏進行參悟(canwu)，靜
觀雲的變化，於太極拳理似乎極有幫助。
LONG );
        set("no_fight", "1");
        set("no_steal", "1");
...
int do_canwu(string arg)
{
        ...
        if (me->is_busy())
                return notify_fail("你現在還忙着，靜不下心來參悟太極拳理。\n");

        if( query("jing", me)<query("max_jing", me)*7/10 )
                return notify_fail("你現在的精氣不夠，無法進行參悟。\n");

        if (lvl < 200)
                return notify_fail("你的太極功夫還不到家，不必在這裏浪費時間了。\n");
        ...
        message_vision("$N盤膝而坐，靜靜地凝視着千變萬化的流雲，開始潛心鑽研"
                       "太極拳理。\n", me);
