...
if( query("family/family_name", me) != "雪山寺"
         ...
         || query("class", me) != "bonze" )
                return notify_fail("你未入佛門，塵緣不斷，無法修持密宗神法。\n");

        message("vision", me->name() + "仔細地研讀《八埋茶剎羅經》。\n", environment(me), me);
        if( query("jing", me)<15){
        ...
        if(me->query_skill("lamaism", 1) < 30){
                write("你研讀了一會兒，但是發現上面所説的對你而言太深奧了。\n");
                return 1;
        }
