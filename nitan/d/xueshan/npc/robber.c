...
int set_random_name()
{
    ...
    set("long", "他看起來是一個年輕的"+menpai_names1[random_id]+"弟子。\n");
...
void self_destruct() {
    ...
    clear_dumudian();
        if (environment(me))
    message("vision", name(1)+"急急忙忙地離開了.\n", environment(me), me);
    destruct(me);
}
...
void start_job(object target)
{
    ...
    if( !me->is_fighting() && !query_temp("disabled", me) )
    {
       ...
       if( present(query("id", target),environment(me))){
           ...
           message_vision(HIR"\n$N對$n大聲喝道：" + RANK_D->query_rude(target) + "竟敢不識好歹？！\n\n"NOR, me, target);
           kill_ob(target);
       }
       else
       {
...
//               EMOTE_D->do_emote(this_object(), vb, emote_arg, 0, 0);
           ...
           else
           {
               ...
               {
                    // set gang on fire.
                    command("say "+"老子看死禿驢們還怎麼做法事");
                    message_vision(HIR"$N從懷中掏出一支火折，點燃了銅缸中的酥油。\n"NOR, me);
                    gang->burn_out_gang();
               }
               self_destruct();
               return;
