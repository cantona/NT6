...
string query_chinese_amount()
{
   ...
   str2 = base2 ? chinese_number(base2) + "鬥" : "";
...
int move_liquid (object target)
{
     ...
     inv = all_inventory(target);
         ...
         if( query("liquid/unique", target) )
            if( query("id", obj) != query("liquid/uniqueid", target) )
                        // you can't pour some liquid into this bottle
                return notify_fail("你可不能把這東西倒入那兒。\n");
     ...
     for( i=0; i<sizeof(inv); i++ )
        {
         ...
         if (liquid_found) {
                ...
                if( query("id", obj) == query("id", liquid) )
                {
