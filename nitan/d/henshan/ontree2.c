...
void create()
{
        set("short", "古樹頂");
        set("long", @LONG
你極目遠眺，四周盡是懸崖峭壁。一條樹籐(teng)從你面前垂下。
...
int do_la(string arg)
{
        ...
        message_vision(NOR + YEL "$N" NOR + YEL "雙手緊緊拉住樹籐，看起來很"
                       "結實。\n" NOR, me);
...
int do_tiao(string arg)
{
        ...
        if (random(6) != 1)
        { 
              message_sort(HIC "\n$N" HIC "雙手緊緊拉住樹籐，騰空而起，向山洞跳去。但是樹籐“啪~”的一聲"
                           "在半空斷掉，$N " HIC "重重地摔了下去……\n\n" NOR, me);
       ...
       message_sort(HIC "\n$N" HIC "雙手緊緊拉住樹籐，騰空而起，向山洞跳去，樹籐在空中劃出一道美妙的弧線。"
                    "$N" HIC "只覺耳邊風聲呼呼……很快$N" HIC "輕輕地落在了洞口，回頭望見樹籐在空中搖擺"
                    "著！\n\n" NOR, me);
