...
void create()
{
        set("short", "伐木場");
        set("long", @LONG
這裏是鹿鳴峯西坡下的一片森林，長着許多百年的參天大
樹，筆直挺拔，都是上好的木材，許多人在這裏砍伐(famu)木料，
然後運到中原供達官貴人們建築豪宅。這裏地處深山老林，時常
有兇猛的野獸出沒。
LONG);
        set("exits", ([
...
void call_beast()
{
        ...
        message("vision", "山坳裏面突然串出" + chinese_number(n) +
...
int do_famu(string arg)
{
        ...
        if (! ob)
                return notify_fail("這裏沒有這個人可以和你一起伐木。\n");

        if (me == ob)
                return notify_fail("你是不是大腦進水...需要修理？\n");

        if (! living(ob))
                return notify_fail("你怎麼也得等人家醒過來再説吧？\n");
        ...
        if( query("qi", me)<200 )
                return notify_fail("你現在氣息不夠，還是養足了力氣再説吧。\n");
...
private varargs int end_working(object me, object passive)
{
        ...
        if (objectp(passive))
        {
                message_vision("$n不幹了，$N也只好跟着停下手來。\n",
                               me, passive);
        }
