...
int do_break(string arg)
{
        ...
        if (query("dao")) return notify_fail("船上的桅桿早就倒了，還砸什麼？\n");
        if (!objectp(inv)) return notify_fail("你準備拿什麼砸船呀？\n");
        else 
        {
        message_vision("$N舉起手中"+query("name", inv)+"，向船上的桅桿砸去。\n",me);
...
{
        message_vision( WHT "\n只聽得喀喇、喀喇之聲不絕，聳立的船桅一根根倒將下來。\n砰□之聲不絕，頃刻之間，眾桅桿或倒或斜，無一得免。\n" NOR,me);
        set("dao",1);
        set("long","這些大船，想是巨鯨幫、海沙派一幹人的座船。只是桅桿都被砸斷了。\n");
        set_temp("dao", 1, me);
        return 1;
}
