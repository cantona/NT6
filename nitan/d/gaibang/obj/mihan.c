...
int response(object me, object who, object obj)
{
        ...
        if( query("target_id", obj) != query("id", who )
         || query("target_name", obj) != who->name()){
                ...
                if( query("shen", who)<0){
                        message_vision("$N姦聲笑道：嘿嘿！人不為己，天誅地滅！天大的富貴送上門來，豈能放過！\n", who);
                        message_vision("$N沉聲說道：我得趕緊向"+boss+"報告丐幫的計劃！\n", who);

                        if( random(query("kar", me))>10){
                                message_vision("$N猙獰地對$n說道：你只好自認倒霉，向閻王訴苦去吧！\n", who, me);
        ...
        if( query("shen", who)<0){
                ...
                if( random(query("kar", me))>10){
                        message_vision("$N沉聲對$n說道：嘿！俗話說兩國交鋒，不斬來使，但算你倒霉！\n", who, me);
                        message_vision("$N姦笑道：我正愁不知要送甚麼給"+boss+"以表達我的心意，你自個兒送上門來，可怨不得我！\n", who);
