...
LONG );
        ...
        set("item_desc", ([
                "stone"          : (: look_stone :),
                "fen"            : (: look_fen :),
                "fir sword"      : NOR + GRN "你拿起這柄劍，只覺着手輕飄飄的渾似無物，原來是柄竹劍。但見劍下的石刻道：\n"
                ...
                "thi piece"      : HIW "石上刻有兩行小字：「凌厲剛猛，無堅不摧，弱冠前以之與河朔羣雄爭鋒。」 \n" NOR,
                "big stone"      : NOR + WHT "一堆石頭，似乎獨孤求敗所遺留之劍就在下面。\n" NOR,
                "lianhua"        : HIG "景色壯麗無比，可以從這裏（jump lianhua）到蓮花峯。\n" NOR,
...
string look_stone()
{
        ...
        msg = NOR + WHT "只見大石上「劍冢」兩個大字之旁，尚有兩行字體較小的石刻：\n"
              HIR "「劍魔獨孤求敗既無敵於天下，乃埋劍於斯。嗚呼！羣雄束手，長劍空"
              "利，不亦悲夫！」 \n" NOR;
...
string look_fen()
{
        ...
        if (! environment(noname) && ! environment(xuantie))
                 msg = NOR + WHT "劍冢下露出並列着的三柄長劍。\n在第一（fir sword）、第二（sec sword）把劍之"
                       "間，另有一塊長條石片（piece）。\n三柄劍和石片並列於一塊大青石之上。\n" NOR;

        else if (! environment(noname) && environment(xuantie))
                 msg = NOR + WHT "劍冢下露出並列着的兩柄長劍，在第一（fir sword）的右邊"
                       "另有一塊長條石片（piece）。\n而第二把劍已不知去向，只留下石片（se"
                       "c piece）上所刻的小字。\n兩柄劍和石片並列於一塊大青石之上。 \n" NOR;

        else if (environment(noname) && ! environment(xuantie))
                 msg = NOR + WHT "劍冢下露出並列着的兩柄長劍。\n在第一（fir sword）、第二（sec sword）把劍之"
                       "間，另有一塊長條石片（piece）。\n而最右首的劍已不知去向，只留下"
                       "石片（thi piece）上所刻的小字。\n兩柄劍和石片並列於一塊大青石之上。 \n" NOR;

        else     msg = NOR + WHT "劍冢下露出並列着的一柄長劍，在第一（fir sword）的右邊"
                       "另有一塊長條石片（piece）。\n其餘的兩把劍已不知去向，只留下"
...
int do_pick(string arg)
{
        ...
        switch(arg)
        {
              ...
              case "sec sword":
                     ...
                     if( query("str", me)<34 )
                           return notify_fail("你將鐵劍只提起數尺，嗆的一聲，竟然脱手掉下"
                                              "，在石上一碰，火花四濺，不禁嚇了一跳。\n");
