...
void create()
{
        set_name(HIR"一堆火燄"NOR,({"huo yan"}));
        set_weight(50);
        if (clonep())
                set_default_object(__FILE__);
        else {
                set("long", HIR"火燄高約尺，"+GRN+"色作純碧，"+BLU+"鬼氣森森，"+HIR+"和尋常火燄大異。\n"NOR);
                set("unit", "堆");
                set("value", 1);
                set("no_get", 1);
                set("no_drop", 1);
...
int do_kick(string arg){
        ...
        if ((damage<150)){
                ...
                message_vision(HIR"\n$N臉色一變，身形一晃，已經轉到$n身後。舉起右腳，微微一挑，將火燄向$n撥去。\n"NOR,me,ob);
                me->start_busy(random(2)+2);
...
void wear(int phase)
{
        ...
        switch(phase) {
        case 1:
                set("long", HIR"火燄熊熊燃燒著，"+GRN+"色作純碧，"+BLU+"鬼氣森森，"+HIR+"和尋常火燄大異。\n"NOR);
