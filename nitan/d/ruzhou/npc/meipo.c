...
{
        message("vision", "突然看見花喜鵲從窗外飛進來，落在媒婆身旁，媒婆笑眯眯地撫着花喜鵲的頭後\n"
                "説道：“請柬剛才已經都送到了！您還想請誰？不想請了就開始結婚手續(marry sb.)吧！”\n", environment(), ob);
          delete("waiting_target", ob);
        delete("me_name", ob);
        delete("me_id", ob);
        return 1;
...
int do_marry(string arg) //判斷結婚
{
        ...
        if( !environment() || base_name(environment()) != query("startroom"))
                return notify_fail("媒婆説道：“真抱歉，我現在很忙，請您等一會到紅娘莊來找我吧。”\n");
   ...
   if( query_temp("marry/money", ob) != me->name( )
      ...
      message_vision("媒婆翻了翻姻緣簿，對$N説道：“婚姻大事怎麼會象你這樣隨隨便便？現在\n"
