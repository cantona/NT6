...
int accept_object(object me, object ob)
{
   if( !query("money_id", ob) )
          ...
          if(ob->value() < 1000000)
              return notify_fail("江耀亭皺了皺眉頭説道：“你記性不好還是怎麼着？100 gold，少一文也別想辦！”\n");
        message_vision("江耀亭接過錢點點説：“好吧！我們來辦理吧！”\n",me);
...
int zou(object me)
{
   message_vision(HIW"一羣衙役衝上前來，不由分説，把$N掀翻在地，一陣大板隨即跟上……\n"NOR,me);
...
int do_kill()
{
        ...
        message_vision("四周的衙役立刻羣起對$N發動攻擊！\n", this_player());
...
int i,*level;
        ...
        me = this_player();
        if (me->is_busy())
                return notify_fail("江知府眉頭一皺説道：“你看看你自己那個忙樣！”\n");
        ...
        if( this_object()->is_fighting() )
                return notify_fail("江知府説道：“你先退下，等本官處理了這裏搗亂的人再説！”\n");

        if( !living(this_object()) )
                return notify_fail("你還是先想辦法把知府大人救醒吧。\n");

        if( !environment()||base_name(environment()) != query("startroom") )
                return notify_fail("知府哼了一聲説道：“急什麼？等本官升堂了再説！”\n");

   if( query_temp("unmarry", me) )
           return notify_fail("江耀亭黑着臉看看你：“本官不是剛剛幫你們辦過離婚手續？還來煩我！？”\n");
...
//        if( !ob->restore() )
   ...
   if(query_temp("money")!=me->name()
           ...
           return notify_fail("江知府拍了拍桌子：“離婚的手續費100 gold你還沒交，沒錢就回去湊合着過日子，不要來煩本官！\n");

        if( query_temp("pending/unmarry", obj) != me )
        {
                set_temp("pending/unmarry", obj, me);
                message_vision(MAG "\n$N看了看$n長歎一口氣道：“罷罷罷……我們還是好聚好散吧！?”\n"
                        "江耀亭眯起眼睛問問$n：“你願不願意離婚呢？説出來，本官為你作主！”\n"NOR,me,obj);
      tell_object(obj,"如果你同意離婚，請你也對"+me->name()+"("+query("id", me)+
                               ")"+"下一次unmarry"+query("id", me)+"的指令。\n");
                write(MAG "現在你正等待着你的伴侶的回答...\n" NOR);
                return 1;
        }
        message_vision(MAG "\n$N對著$n哼了一聲説道：“事已至今，既然無緣，咱們就各走各的路吧。”\n\n"NOR, me,obj);
   set_temp("unmarry", 1, me);
   set_temp("unmarry", 1, obj);
        message_vision(HIY "江耀亭看了看點點頭説道：“$N和$n，既然你們都同意離婚，那本官也就給你們辦了！”\n"
           "説完，在姻緣簿上重重地劃去了兩個名字。\n" NOR, obj, me);
        CHANNEL_D->do_channel(this_object(), "mudnews",
