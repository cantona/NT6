...
void create()
{
        set("short", "攬月居");
        set("long", @LONG
這是慕容公子的卧室，房中陳設普通，收拾得頗為整潔，屋角一
架茂蘭盆景(penjing)長得很是旺盛。晚上月光從窗户射進來，滿室銀
光，是燕子塢的一大美景。
LONG );
        set("exits", ([
            "west": __DIR__"shuwu",
...
int check_pass(object me)
{
       write(HIC "不一會兒，只聽不遠處隱約傳來一陣説話聲：“灑國夢語…”\n" NOR);
       write(HIY "你想了想，覺得像是一句暗語，你很想回答(whisper)但又不知道説些什麼！\n" NOR);
...
int do_say(string arg)
{
     ...
     if( !arg || !query_temp("can_whisper", me) )
              return notify_fail("你自言自語不知道在説什麼！\n");

     if(arg != "光復大燕")
     {
              write(HIG "你悄悄地説道：“" + arg + "”，卻沒什麼反應。\n");
              return 1;
     }
 
     write(HIG "你悄悄地説道：“光復大燕”……\n" NOR);
     write(HIC "裏面的人笑了笑説道：“既然是主人的吩咐，那你就進來吧！”\n");
     write(HIY "忽然，轟轟幾聲過後，花盆後露出一條路來，你趕忙順着走了下去……\n");
