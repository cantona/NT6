...
int do_chushi(string arg)
{
        ...
        if( !quest=query("quest_hs", me) )
            return notify_fail("你沒有接到徵收的任務，在這裏用英雄令晃啊晃的幹啥？\n");
        ...
        if( query_temp("quester", who) != query("id", me) || quest["target"] != query("id", who) )
            return notify_fail("你不要仗着武林同盟的名聲到處招搖撞騙！\n");
        message_vision("$N高舉武林令，對着$n大聲説：“你這個傢伙，準備躲着武林盟主多久？\n"
                       "好在今天讓我找到了你！黃大人讓我來收取"+quest["name"]+"，你就乖乖的交出來吧！\n",me,who);
        message_vision("$N見到$n手中的武林令，不由一聲長歎：“躲了這麼久，還是給你們找到了，也罷也罷！\n"
                       "你要的東西我這裏有，就交給你了。\n",who,me);
