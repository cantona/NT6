...
nomask void start_perform_busy(int new_busy)
{
        ...
        if( me->is_fighting() ) {
                avd_busy = query_temp("apply/avoid_busy");
                rdc_busy = query_temp("apply/reduce_busy");

                if( objectp(ob = query_temp("last_opponent") ) &&
                    me->is_fighting(ob) )
                        add_busy = query_temp("apply/add_busy", ob);
        }
...
varargs void start_busy(mixed new_busy, mixed new_interrupt)
{
        ...
        */
        ...
        if( intp(new_busy) ) {
                ...
                if( new_busy > 1 && avd_busy > 0 && random(100) < avd_busy )
                        new_busy = 0;
