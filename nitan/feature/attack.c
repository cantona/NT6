...
void reset_action()
{
        ...
        if( objectp(ob = query_temp("secondary_weapon")) && (ob->query("skill_type") != "throwing")) {
                if( ob->query("skill_type") != type ) // 是否為不同種兵器
                {
                        if( stringp(skill = query_skill_mapped(ob->query("skill_type"))) &&
                            (int)query_skill(skill, 1) > 0 ) 
                                set("second_actions", (: call_other, SKILL_D(skill), "query_action", me, ob :));
                        else
                                set("second_actions", query("actions", ob));
                }
                else
