...
string ask_where()
{
    ...
    return me->query_temp("quest/name")+CYN"是吧？聽説有人看見他曾經出現在"+name+CYN"一帶。"NOR;
}
...
int accept_object(object who, object ob)
{
        ...
        if (!who->query_temp("ask_chance") && ob->query("money_id") && ob->value() >= 5000)
        {
                ...
                if (per > 25) {
                        write("算命先生景仰地説：“看" + RANK_D->query_respect(who) + "相貌堂堂，天庭飽滿，地廓方圓，");
                        ...
                        write("算命先生清了清嗓子，説：“看" + RANK_D->query_respect(who) + "相貌屬中上之資，");
                        ...
                        write("算命先生有點勉強地説：“看" + RANK_D->query_respect(who) + "相貌平平，倒也還能算是中人之資，");
                        ...
                        write("算命先生很厭惡地説：“看" + RANK_D->query_respect(who) + "五官挪位，印堂發暗，");
                ...
                if (who->query_temp("ask_chance") && ob->query("money_id") && ob->value() >= 100000) {
                        write("算命先生當即説道：“好説，好説，一切都好説。”\n");

                        tc = who->query("combat_exp")/1000000;
                        write("算命先生説道：“看" + RANK_D->query_respect(who) + "的命相，當前每個謎題總共可有" + chinese_number(tc) + "次機會。”\n");

                        if (!who->query("quest/jiuyin1/pass")) {
                                temp = tc - who->query("quest/jiuyin1/fail");
                                if (temp < 0) temp = 0;
                                write("算命先生説道：“" + RANK_D->query_respect(who) + "還有" + chinese_number(temp) + "次機會，解開九陰真經上冊。”\n");
                        }
                        ...
                        if (!who->query("quest/jiuyin2/pass")) {
                                ...
                                if (temp < 0) temp = 0;
                                write("算命先生説道：“" + RANK_D->query_respect(who) + "還有" + chinese_number(temp) + "次機會，解開九陰真經下卷。”\n");
                        }

                        if (!who->query("jiuyin/emeipass") && (who->query("family/family_name") == "峨嵋派")) {
                                temp = tc - who->query("jiuyin/emeifail");
                                if (temp < 0) temp = 0;
                                write("算命先生説道：“" + RANK_D->query_respect(who) + "還有" + chinese_number(temp) + "次機會，解開峨嵋九陰真經。”\n");
                        }
                        if (!who->query("quest/qianzhu/pass") && (who->query("gender") == "女性" )) {
                        	      tc = who->query("combat_exp")/500000;
                                temp = tc - who->query("quest/qianzhu/fail");
                                if (temp < 0) temp = 0;
                                write("算命先生説道：“" + RANK_D->query_respect(who) + "還有" + chinese_number(temp) + "次機會，解開千蛛萬毒手。”\n");
                        }
                        ...
                        if (!who->query("quest/jiuyingm/pass") && (who->query("family/family_name") == "古墓派")) {
                                ...
                                if (temp < 0) temp = 0;
                                write("算命先生説道：“" + RANK_D->query_respect(who) + "還有" + chinese_number(temp) + "次機會，解開古墓九陰真經。”\n");
                        }
                        ...
                        if (!who->query("quest/jiebai/pass")
                            ...
                            && (int)who->query("combat_exp") >= 1000000) {
                                ...
                                if (temp < 0) temp = 0;
                                write("算命先生説道：“" + RANK_D->query_respect(who) + "還有" + chinese_number(temp) + "次機會，和老頑童結拜。”\n");

                                temp = tc - who->query("quest/hubo/fail");
                                tc = who->query("combat_exp")/500000;
                                if (temp < 0) temp = 0;
                                write("算命先生説道：“" + RANK_D->query_respect(who) + "還有" + chinese_number(temp) + "次機會，學會雙手左右互搏。”\n");
                        }
...
/*                       if (!who->query("lbwb/lbwb1pass") && !who->query("lbwb/lbwb2pass")) {
                                ...
                                if (temp < 0) temp = 0;
                                write("算命先生説道：“" + RANK_D->query_respect(who) + "還有" + chinese_number(temp) + "次機會，在無量玉洞學到凌波微步。”\n");

                                temp = tc - who->query("lbwb/lbwb2fail");
                                if (temp < 0) temp = 0;
                                write("算命先生説道：“" + RANK_D->query_respect(who) + "還有" + chinese_number(temp) + "次機會，從段譽處學到凌波微步。”\n");
                        }
*/
                        ...
                        write("算命先生説道：“" + RANK_D->query_respect(who) + "有" + chinese_number(tc) + "次打造機會，已經用了" + chinese_number(temp) +"次了。”\n");
