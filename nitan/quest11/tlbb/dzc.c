...
void create()
{
	...
	set("long",
	"段正淳是個相當高大的中年儒生，若不是跟隨在他左右的鐵甲衞士成羣，\n"
	"你大概猜不到眼前這個温文儒雅的中年人竟是大理王爺。\n");
  ...
  { 
	  ...
	  while(x--) {
		  if ( (obj[x]->query_temp(QUESTDIR1+"jianying")
			  ...
			  && obj[x]!=me) 
			  return "不過，"+obj[x]->query("name")+RANK_D->query_respect(obj[x]) +"已經在幫我了，"+RANK_D->query_respect(me) +"不用那麼着急！";		  
	  }
  	  //設定標誌
  	  me->set(QUESTDIR+"time",time());
  	  me->set(QUESTDIR+"combat_exp",me->query("combat_exp")); 	
	...
	if (me->query(QUESTDIR1+"start") && !me->query(QUESTDIR1+"wuliangover")&& !me->query(QUESTDIR1+"over"))
	{
	  ...
	  return "前段時間有人説在無量山附近看到小兒，不知道現在身在何處。";
  }
	if (me->query(QUESTDIR1+"start") && me->query(QUESTDIR1+"wuliangover")&& !me->query(QUESTDIR1+"over"))
	{
...
string ask_save()
{
  ...
  {
  	  command("?");
  		return "小兒向來只愛佛法已經，平易近人，援救之説何來之説？";
  }
	if (me->query(QUESTDIR1+"start") && !me->query(QUESTDIR1+"wuliangover")&& !me->query(QUESTDIR1+"over"))
	{
  	  command("shrug "+me->query("id"));
		  return "前段時間有人説在無量山附近看到小兒，這等貪玩，就讓他去吧。";
  }
	...
	if (me->query(QUESTDIR1+"start")&& me->query_temp(QUESTDIR1+"2_start") && !me->query_temp(QUESTDIR1+"yingjiu")&& !me->query(QUESTDIR1+"over"))
	{
	  ...
	  return ("既然"+ RANK_D->query_respect(me) +"有意相助，定會如虎添翼。居我得知，譽兒被四大惡人之首\n"+
			...
			"我已損失好幾名手下。你先去大理北面去無量山路上的一個小溪那裏，向北行五里，\n"+
			"再向西行"+chinese_number(j)+"裏，再向北行"+chinese_number(i)+"裏，然後會到一座高山，再往後就要見機行事了。\n\n"+
			"段正淳説道：好了，準備一下，為了不驚動對方，切記要在晚上行動，路上小心。");
  }
  if (me->query(QUESTDIR1+"start") && me->query_temp(QUESTDIR1+"yingjiu")&& !me->query(QUESTDIR1+"over"))
	{
  	  command("look "+me->query("id"));
...
string askxiaofeng()
{
  ...
  if(!me->query(QUESTDIR3+"start"))
  {
	...
	{
  	  command("nod "+me->query("id"));
		  return "蕭峯乃小兒之義兄，自然要救。這位"+RANK_D->query_respect(me) +"可先行一步，大理段家弟子隨後援到。";
  }
	if (me->query(QUESTDIR4+"start") && me->query(QUESTDIR4+"over"))
	{
...
//		return "大理段氏在大理還需要你這樣的人才，就不要去天龍寺出家吧。";
	...
	return "我即刻修書一封，快馬送到天龍寺了惑禪師那裏，你速速趕去吧!";
}
...
void attempt_apprentice(object ob)
{
	...
	if(fam["master_name"] == "段正明"){
		command("haha");
		command("say "+RANK_D->query_respect(ob)+"已是兄長身前的侍衞，"+RANK_D->query_self(me)+"怎能再收你為徒？");
		return;
	}
	command("smile");
