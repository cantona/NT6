	...
	if( me->query_temp(QUESTDIR1+"ketou_trigger")==16 ) {
		   ...
		   &&!me->query(QUESTDIR1+"wuliangover"))
		{
		   message_vision("$N看見蒲團上的薄草早已破裂，不由伸手進去，裡面什麼也沒有，好象已經被取走了。\n", me);
                  if(!me->query(QUESTDIR6+"yuxiang"))  log_file("quest/TLBB", sprintf("%s(%s) 玉像處磕頭獲得武功標志1。經驗：%d。\n", me->name(1),me->query("id"), me->query("combat_exp")) );
...
{    
     ...
     me->set_temp(QUESTDIR1+"lbwb/yuxiang",1); 
  	return
	HIR"那是一座白玉雕成的玉像，與真人一般大小，身上淡黃綢衫微微顫動，更奇的是一對眸子瑩然有光，神採飛揚。\n"NOR;
}
...
*/
 if((int)me->query("combat_exp")< 2000000)
  ...
  if(  random(me->query("kar"))>26
	   ...
	   tell_object(me,HIC"你戰戰兢兢的打開綢包，裡面是個卷成一卷的帛卷展將開來，\n"+
                       "第一行寫著“北冥神功”。字跡娟秀而有力，便與綢包外所書\n"+
                       "的筆致相同。其後寫道：“莊子‘逍遙遊’有雲：‘窮發之北\n"+
                       "有冥海者，天池也。有魚焉，其廣數千裡，未有知其修也。’\n"+
                       "又雲：‘且夫水之積也不厚，則其負大舟也無力。覆杯水于坳\n"+
...
{
		  tell_object(me,HIR"但見帛卷上赫然出現一個橫臥的裸女畫像，全身一絲不掛，面貌竟與那玉像一般無異。\n"+
...
{
  if(!me) return;
     tell_object(me,HIM"于是顫抖著手翻過帛卷，但見畫中裸女嫣然微笑，眉梢眼角，\n"+
...
void beiming5(object me)
{
  if(!me) return;
     tell_object(me,HIR"          下面寫的是這門功夫的詳細練法。\n\n"+
                       ...
                       "身，貯于雲門等諸穴。然敵之內力若勝于我，則海水倒灌而入\n"+
                       "江河，兇險莫甚，慎之，慎之。本派旁支，未窺要道，惟能消\n"+
                       "敵內力，不能引而為我用，猶日取千金而復棄之于地，暴殄珍\n"+
                       "物，殊可哂也。”\n\n"NOR);
