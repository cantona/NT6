...
void create()
{
        set("short", "聚賢莊大門");
        set("long",
"你走到一座高宅大院的門口，大門開着，門前左右有兩個石獅子，四五級\n"
"青石台階上是兩扇釘滿銅釘的大門。看得出這裏是一大户人家的住所。門口正\n"
"中央高懸着一塊黑底金字的大匾，匾上寫有三個大字："YEL"【聚賢莊】"NOR"。\n"
        );
		set("quest",1);
		set("outdoors", "南陽");
		set("exits", ([
...
int valid_leave(object me, string dir)
{
   ...
   if (me->query(QUESTDIR3+"start") 
     ...
     return notify_fail(HIR"管家冷笑一聲道：這位"+RANK_D->query_respect(me) +"請回，這裏不歡迎你！\n"NOR);
     if (me->query(QUESTDIR3+"start") 
         && !me->query(QUESTDIR3+"over") 
         && dir == "north" 
         && present("jia ding", environment(me)))
     return notify_fail(HIG"家丁冷笑一聲道：這位"+RANK_D->query_respect(me) +"請回，這裏不歡迎你！\n"NOR);

     if(dir == "north" &&(me->query_condition("killer")||	sizeof(filter_array(deep_inventory(me), (: userp :)))))
		 return notify_fail(HIY"突然從裏面傳來一陣冷笑道：這位"+RANK_D->query_respect(me) +"請回，這裏不歡迎你！\n"NOR);
