...
int do_jump(string arg)
{
	...
	message_vision(HIC"$N一躍而起，環顧四周，這裏竟然就是遼營的門前那片樹林。\n"NOR, me);
	me->start_busy(1);
	return 1;
} 
int do_dianhuo(string arg)
{
	...
	if (present("liao bing", ob))
		return notify_fail("遼兵就在這裏，先解決掉遼兵再説。\n");
	if (me->is_busy() || !arg )
		return notify_fail("你等等想好了燒什麼再説。\n");    
	if (arg!="liang cao" && arg!="liangcao" && arg!="cao" )
		return notify_fail("你等等想好了燒什麼再説。\n"); 
	if(me->query_temp("quest/天龍八部/大遼救援篇/dianhuo"))
		return notify_fail(HIR"火勢已經上來，趕快離開這裏去搭救蕭峯。\n"NOR);    
