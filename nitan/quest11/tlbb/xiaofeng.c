...
void init()
{
	...
	if (interactive(me) && living(me)) {
		...
		if(!me->query_condition("killer")&& me->query(QUESTDIR4+"start") && me->query_temp(QUESTDIR4+"breakmen")&&!me->query_temp(QUESTDIR4+"helpstart") &&!me->query_temp(QUESTDIR4+"follow")&& !me->query(QUESTDIR4+"over"))
		{
			...
			message_vision(HIY"$n進來時，$N靠在鐵籠的欄桿上，神色倒是正常。\n看暗室一旁酒壇堆積，感覺$N這裡的日子竟也是好酒好飯，禮數不缺。\n"NOR,this_object(),me);   		
			command("say 沒想到，又是"+RANK_D->query_respect(me) +"舍命相救。"); 
...
void greeting(object me)
{
	...
	if(me->query(QUESTDIR3+"start") && me->query_temp(QUESTDIR3+"lookcliff")&& me->query_temp(QUESTDIR3+"over") && !me->query(QUESTDIR3+"over"))
	{
        ...
        tell_object(me,HIY"\n你于"+NATURE_D->game_time()+"終于幫助蕭峰確定身世之謎，獲得"+i+"點最大內力、"+me->query_skill("force")/2+"點經驗和"+(int)me->query_skill("force")/5+"點聲望。\n\n"NOR);
...
void check_answer(object me,int i)
{
	...
	if(me->is_fighting(ob)||ob->is_fighting(me)) 
	{
		...
		message_vision(HIW"$N對$n冷笑一聲，竟然趁我之危！如此小看于我。\n"NOR,ob,me);
...
int ask_save()
{
		...
		&& !me->query(QUESTDIR4+"over"))
	{
		...
		log_file("quest/TLBB", sprintf("%s(%s)準備沖出大遼，特此紀錄。經驗：%d。\n", me->name(1),me->query("id"), me->query("combat_exp")) ); 
...
void check_me_where(object me)
{
	...
	else
	{
		...
		message_vision(HIW"$N喟然吟道：“烽火燃不息，征戰無已時。野戰格鬥死，敗馬號鳴向天悲。鳥鳶啄人腸，沖飛上掛枯枝樹。士卒塗草莽，將軍空爾為。”\n"NOR,ob);
		...
		if(!me->query(QUESTDIR4+"over")) 
		{
			...
			tell_object(me,HIY"\n你于"+NATURE_D->game_time()+"終于將蕭峰從大遼解救出來，獲得"+i+"點最大內力、"+(me->query_skill("force")+500)+"點經驗和"+(int)me->query_skill("force")/3+"點聲望。\n\n"NOR);
...
int ask_skills()
{
    ...
    if(me->query("quest/天龍八部/武功/quanli"))
    {
		...
		message_vision(HIY"\n$N向你呼的一拳打出，$n急忙應招。兩人所使的拳招，都是一般的平平無奇，但$N每一招都是\n"
			             +"慢了一步，任由$n先發。$n一出招，$N跟著遞招，也不知是由于他年輕力壯，還是行動加倍的\n"
						 +"迅捷，每一招都是後發先至。這“太祖長拳”本身拳招只有六十四招，但每一招都是相互克制，\n"
						 +"$N看準了對方的拳招，然後出一招愉好克制的拳法，$n焉得不敗？\n\n"NOR,this_object(),me);
		command("haha "+me->query("id"));
		return 1;
	}
    if(!me->query(QUESTDIR3+"over"))
	...
	message_vision(HIY"\n$N驀地心念一動，呼的一拳打出，一招“沖陣斬將”，也正是“太祖長拳”中的招數。這一招\n"
		             +"姿工既瀟洒大方已極，勁力更是剛中有柔，柔中有剛，武林高手畢生所盼望達到的拳術完美之\n"
					 +"境， 竟在這一招中青露無遺。\n"NOR,this_object());
...
int ask_truth()
{
    ...
    if(me->query(QUESTDIR3+"start")&&me->query_temp(QUESTDIR3+"help")&&!me->query_temp(QUESTDIR3+"askxiao"))
    {
    	...
    	command("sigh");
    	command("say 我受傷未能遠行，還請這位"+RANK_D->query_respect(me) +"去一次頌摩崖，那裡據說有一塊刻字記載當年事跡。");
    	command("shout");
    	command("thank "+me->query("id"));
...
void xiaofeng_guiyin(object me)
{
  ...
  if(me->query_temp(QUESTDIR3+"good") && !me->query(QUESTDIR3+"good") && me->query(QUESTDIR3+"over")&& !me->query(QUESTDIR3+"bad"))
  {
    ...
    tell_object(me,HIY"\n\n你于"+NATURE_D->game_time()+"選擇勸說蕭峰歸隱江湖，也算功德無量，獲得"+(int)me->query_skill("force")/4+"點聲望的獎勵。\n\n");
...
void die()
{
  ...
  if(ob->query("kill_id")
	  ...
	  && me->query(QUESTDIR3+"bad"))
	{
		...
		i = 10+random(20);
		me->add("max_neili",i);
		tell_object(me,HIY"\n\n你于"+NATURE_D->game_time()+"選擇為武林除害，擊敗蕭峰，獲得"+i+"點內力的獎勵。\n\n");
