...
void create()
{
	set("short", "廣場");
	set("long", @LONG
這裏是少林寺前的廣場，整個廣場由大塊的青石鋪成，極為平坦。但因年
代久遠，都有些破損。叢叢小草從石板的縫隙中長了出來。廣場周圍都是密密
的松樹林，四角上各放了幾塊石碑，字跡都甚為模糊。正前方黃牆碧瓦，飛檐
翹檁，正中一道二丈來寬，三丈來高的硃紅杉木包銅大門(gate)。上方一塊大
匾，龍飛鳳舞地書着『[1;33m少林寺[0;37;0m』三個大字。寺前三三兩兩的站着幾個僧人。
LONG
    );
	...
	if ( me->query("family/family_name") == "少林派" ) {
		if ( me->query("guilty") == 0
		 && me->query("K_record") == ( me->query("PKS") + me->query("MKS"))) {
			...
			write("壯年僧人沉下臉來，説道：戒律院主持玄寂大師請師兄火速去戒律院陳述此行過犯。\n");
			return 1;
		}
	}
		...
		message_vision(HIY "壯年僧人對$N合十為禮，側身讓開，説道：原來是闖過羅漢陣的大英雄駕到，請進請進！\n" NOR, me);
		return 1;
	}
	if (me->query("shen") >= 0) {
