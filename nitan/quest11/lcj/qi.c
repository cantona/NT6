...
int ask_xuexi4()
{
	...
	if(ob->query_temp("jobask") == 4) {
		...
		if(ob->query_temp("jobover") != 4) {
			...
			command("say 聽説我師兄還有點事情找你，你最好去看一下。"NOR);
	...
	if ( ob->query_temp("jobover") == 4) {
		command("say 我不是都和你説了麼，我師兄有事找你，快去看下吧。");
		return 1;
	}
}
string ask_diyun()
...
string ask_tangshi()
{
	...
	command("consider");
	message_vision(HIY"$N仔細在腦海裏搜索“唐詩選輯”。\n", ob);
	command("look "+me->query("id"));
	command("say 恩，好像有這麼個名字。可是我有點忘記在哪裏了。");
...
string ask_xieyang()
{
	...
	command("en");
	message_vision(HIY"$N慢慢在腦海裏搜索“唐詩選輯”、“鞋樣”，突然似乎想了起來。\n", ob);
	command("look "+me->query("id"));
	command("say 恩，好像有這麼個名字。我記得給狄雲用來做過鞋樣的。");
	command("say 我好像順手隔在書房裏。只是也不知道現在狄雲怎麼樣了。");
