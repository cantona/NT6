...
void greeting(object ob)
{
	...
	if ( ob->query("combat_exp") > 3500 && !wizardp(ob) && !ob->query_temp(QUESTDIR2+"start") ){
		...
		command("say 這位" + RANK_D->query_respect(ob)+ "，你已經能到外面的江湖上去闖盪一翻了，怎麼還想\n"+
			"在武館裡偷懶？武館不希望你再留在此地了。\n");
	...
	if( ob->query_temp("jobover") == 1) {
		...
		command("whisper "+ ob->query("id")+ " 現在對自己和武館的環境有一定了解了吧？不過，\n"+
			"光了解周圍的環境還不夠，還要了解整個世界的情況才行，周圻師兄常年闖盪\n"+
	...
	if( ob->query_temp("jobover") == 2) {
		...
		command("whisper "+ ob->query("id")+ " 了解了整個世界，是不是感覺雄心勃勃，想到江湖\n"+
	...
	if( ob->query_temp("jobover") == 4 && ob->query_temp("jobask") != 5) {
		...
		command("nod");
		command("whisper "+ ob->query("id")+ " 好了，現在你已經對書劍也了解的差不多了。武館\n"+
...
int do_answer(string arg)
{
	...
	if ( me->query_temp("wgmeet")) {
		...
		if ( arg == "y" ) {
			...
			command("nod "+ me->query("id"));
			command("whisper " + me->query("id")+ " 看來你對書劍已經有了一定的了解，武館裡還有很\n"+
