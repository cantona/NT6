...
int ask_fuwei()
{
	say(CYN"林震南炫耀地説：「我們林家三代走鏢，一來仗着先祖遠圖公當年闖下的威名，\n"
	"二來靠著我們林家家傳的玩藝兒不算含糊，這才有今日的局面，成為大江以南\n"
	"首屈一指的大鏢局。江湖上提到『福威鏢局』四字，誰都要翹起大拇指，説一\n"
	"聲：『好福氣！好威風！』哈哈，哈哈！」\n"NOR);
...
int ask_yuantu()
{
	if (this_player()->query_temp("marks/林1")) {
		say(CYN"林震南磕一磕煙斗，説道：「遠圖公是我的祖父，福威鏢局是他一手創辦的。\n"
...
int ask_pixie()
{
	...
	if (this_player()->query("combat_exp") < 100000) {
		...
		if (this_player()->query("quest/pixie/pxj_passwd"))
			say(CYN"林震南不悦地説道：「我不是都告訴你了嗎？我的劍法不及先祖。」\n"NOR);
		else if (this_player()->query_temp("marks/林2")) {
			say(CYN"林震南一驚，説道：「你怎麼知道的？噢，我剛才説露嘴了。嗨，在下的功夫\n"
			"外人不知底細，其實及不上先祖。」\n"NOR);
			write(CYN"林震南頓一頓，接着低聲説道：「先祖去世前，曾給家父留下");
			p1=random(4)+1;
			p2=random(4)+1;
			p3=random(4)+1;
			p4=random(4)+1;
...
int ask_tong()
{
	...
	if (!(p=this_player()->query("quest/pixie/pxj_passwd")))
		say(CYN"林震南一臉譏笑的樣子，説：「怎麼混到這一步了到處要錢？」\n"NOR);
	else {
		write(CYN"林震南低聲説：「先祖去世前，曾給家父留下");
