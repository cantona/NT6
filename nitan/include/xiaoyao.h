...
string fam_type(object ob)
{
        ...
        if (!ob->query("family") || 
        ob->query("family/family_name")!="逍遙派")        return 0;
        if (!objectp(weapon= ob->query_temp("weapon")))
        {
                ...
                && xiaoyao_master(ob) )
                return "xiaoyao";
                if (ob->query_skill_mapped("strike")=="liuyang-zhang"
                &&  ob->query_skill_mapped("force")=="bahuang-gong"
                &&  ob->query_skill_mapped("dodge")=="yueying-wubu"
                &&  ob->query_skill_prepared("strike")=="liuyang-zhang"
                && lingjiu_master(ob) )
                return "lingjiu";
                return 0;
        }
        ...
        && ob->query_skill_mapped("strike")=="liuyang-zhang"
        &&  ob->query_skill_mapped("force")=="bahuang-gong"
        &&  ob->query_skill_mapped("dodge")=="yueying-wubu"
        &&  ob->query_skill_prepared("strike")=="liuyang-zhang"
        && xiaoyao_master(ob) )
