...
void hjset_short()
{
    ...
    set("color_short_hj",cl1+"遙"+cl2+"遠"+cl3+"傳"+cl4+"説"+cl5+"之"+cl6+"起"+cl7+"緣"NOR);
...
void hjset_long()
{
  ...
  巨大的木板(ban)飄忽地於你身前不遠之處隱現，巨板下帶着一塊小木牌(pai)。
這裏便是傳説中的“幻境”與現實之間的接口，由此而進，你將隨之步入那神祕的
                          幻境之地，不復存在……
...
int do_enter_hj(string arg)
{
    ...
    tell_room(rooms,"忽然一陣旋風襲過，"+query("name", me)+"已出現在這裏。\n",me);
    return 1;
}
*/
...
其中一個樣貌古怪的女子説對着$N説道：“恭喜恭喜！幻境正在發放特別獎勵，你是這次的幸運者！”
另外一個比較像正常人的男子接着説道：“你若要進入幻境的話，將獲得特別的獎勵！”
只見他手一揮似的動作，$N覺得微微一震，就像被什麼東西輕輕碰撞了一下一般！
這幾個人忽來忽去，説話間竟然就此退下，往裏面進去了。\n\n",me);
...
常年發售遊戲説明手冊，價格 "HIR"1"NOR" GOLD。指令： "HIY"(goumai shuoming)"NOR"
玩家可以發佈信息，價格 "+HIR+MSG_GOLD+NOR" GOLD。指令："HIY"(hjmsg)"NOR"
玩家可以查詢誰在幻境遊戲裏，指令："HIY"(gwho)"NOR"
...
int do_goumai(string arg)
{
    ...
    // 已刪除此物件
    return notify_fail("請直接輸入 help hj2003 查詢遊戲説明。\n");

    if(!arg || arg!="shuoming")
        return notify_fail("購買遊戲説明手冊，價格1 GOLD，指令 <goumai shuoming> 。\n");

    gold = present("gold_money",me);
    if(!gold) return notify_fail("你身上沒有黃金。\n");
    gold->add_amount(-1);
    new(__DIR__"other_obj/obj_shows")->move(me);
    message_vision("$N掏出一兩黃金，買下了一本遊戲説明手冊。\n",me);
    in_money ++;
    buy_sm_times ++;
    save();
    return 1;
...
dellv ID        --> 清除某玩家在這裏的 level 記錄(並不清除其實際有效 level)\n");
...
總收入："HIY"%d"NOR" 黃金 (售票 %d 張，發佈信息 %d 次，售出説明 %d 本)
(門票 %d GOLD / 張，説明 1 GOLD / 本，發佈信息 %d GOLD / 次)
************************************************************\n",
...
int valid_leave(object me, string dir)
{
    ...
    else
    {
        ...
        if( !menpiao || !query("hjmenpiao", menpiao) )
            return notify_fail("請先購票，方可入內。請查看牌子上的説明。\n");
        destruct(menpiao);
        message_vision("$N掏出一張門票塞進門邊的小盒子裏，隨即往裏邊走了進去。\n",me);
        in_times++;
        save();
    }
...
void player_timeout(object me)
{
    ...
    message_vision( "$N呆在這裏太久了，忽然感到一陣震盪！\n",me);
    me->move( this_object() );
    message_vision( "$N被一陣莫名的震盪送到了這裏。\n",me);
}
