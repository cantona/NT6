...
// naihe 05-9-1 16:04 debug, 購買時居然不給錢給委托者，煩躁
...
發布求購信息："HIY"(qiugou ID)"NOR"
購買貨品：    "HIY"(goumai ID)"NOR"

代售貨品、發布求購信息都需要"HIR"手續費 "+SXF+" 點"NOR"（以你的幻境得分兌現）。
代售貨品時，如果原物主已離開幻境或已死亡，將會取消代售，貨品將扔在本地。
發布求購信息後，將于半小時後清除；或在此期間若有該ID的新貨到來，將在通知後清除。\n\n");
...
int do_qiugou( string arg )
{
    ...
    message_vision( CYN"$N"NOR+CYN"拿出一點什麼塞給了$n"NOR+CYN"，說道：“我急需 \""+arg+"\" 這個貨，如果有消息請盡快通知我！”\n"NOR, me, this_object() );
    command( "nod2 "+id );
...
"你有什麼東西不需要的，來看看別人是不是正在求購它，也好賣掉啊！(list qg)",
    ...
    if( query("new_goods") && environment( query("new_goods") ) == this_object() )
    {
        ...
        if( temp && sizeof(temp) == 3 )
        {
            i=3;  // 讓新貨消息發布的頻率高些
            while(i--) 
            msg += ({ sprintf( "現在有%s委托的最新貨品%s"NOR+CYN"，僅售 %s 點，快來看看啦！",
