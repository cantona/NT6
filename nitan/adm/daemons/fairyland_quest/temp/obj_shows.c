...
void create()
{
    set_name("『幻境·遙遠傳説之起緣·幽靈顯現』 玩家手冊", ({"shou ce","ce"}));
    set_weight(10);
    if (clonep())
        set_default_object(__FILE__);
    else
    {
        set("long", "這是一小本關於“幻境”遊戲的玩家説明手冊，輸入 <shows> 查看説明。\n");
        set("unit", "本");
        set("value", 0);
    }
...
{
    if(!arg) return notify_fail("你可以輸入頁數或類型來閲讀這些説明。例如 <shows 1> 或 <shows story>。
...
『幻境·遙遠傳説之起緣·幽靈顯現』  劇情
    ...
    熱情的勇士仍在詭祕的密林中不懈尋找，而林中也日漸熱鬧，
各色人等穿梭往返，但卻一直徒勞無功。

    正當所有的人們都失望消散之時，一個月色幽朗的夜晚，你
在偶然的走動之中，竟來到了林中一處從未見到過的水潭之岸，
在岸邊精緻幽雅的小亭前方，一柄粗大的權杖插在地上。你看着
權杖上刻着的小字，一股寒意從心底漸漸升起……

    幽靈之族，終於此時而始，來到了這個神祕的樹林之中！
...
write("
    遊戲裏以得分為主要目標，它並不限制你必須做什麼和必須不做什麼，
...
果你的氣息已經降至零點或以下，那麼即使有再多的得分，也將全部作廢。
    ...
    而遊戲的指令，請在進入遊戲後，輸入(help hj) 查看詳細的説明。
***************************************************************\n");
return 1; }
    ...
    {
write("****************************************************************
『幻境·遙遠傳説之起緣·幽靈顯現』 支線任務介紹

    在 『幻境·遙遠傳説之起緣·幽靈顯現』 裏，除了尋找自己的寶物
之外，主要的得分途徑還有一個，這就是支線任務。由於在『幽靈顯現』
裏增加了許多個支線任務，並且它們都帶有一定的謎題性質，所以在這裏
...
return 1; }
    ...
    {
write("****************************************************************
『幻境·遙遠傳説之起緣·幽靈顯現』 非玩家操作類人物(NPC)介紹

    1、自己的NPC
    每個進入遊戲的玩家都會對應一個不同的特殊npc，即臣民(chen min)。
他身上帶着你要找的寶物的最重要的那部分，你要向他打聽寶物的名字，
...
都不相同。攻擊NPC能夠得到遊戲裏的得分，殺死它們還有可能得到一些物品。
...
return 1; }
    ...
    {
write("****************************************************************
『幻境·遙遠傳説之起緣·幽靈顯現』 技能、力量的保留與人物升級

    在『幽靈顯現』裏，你在遊戲中積累的力量值以及所有的技能，都能夠得
到保留，即你在下一次進入遊戲時，不必從一個什麼技能都不懂、力量低微的
“弱小狀態”開始遊戲，而可以保留你大部分在之前通過練習而獲得的這些較
高的數值進入遊戲。

    保留的方法是：在任何一次成功遊戲後，兑換獎勵時選擇相應的項目即可。
注意，你必須要有相當高的得分，才能夠兑換這類的獎勵。一些補充説明，請
