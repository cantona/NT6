...
void remove()
{
    ...
    {
        if(m["obj"])
        tell_object(m["obj"],"重新更新 im 系□，□重新登入。\n");
...
void login_irc(object user)
{
    ...
    if(!id)
    {
        tell_object(user,"你□有登入 ID ");
        return ;
    }
    fd = socket_create(STREAM, "read_callback","close_socket");
    if ( fd < 0 )
    {
        tell_object(user,"□□失□，可能是主□□有□□或是□路□法□□。");
        return;
    }
    ...
    {
#ifdef DEBUG
    TELL(sprintf("[%s] %s 成功□□ Socket ,□始□□登入 irc.fd = %d\n",
         ctime(time()) ,id,fd));
#endif 
    }
    else
...
private int process_identify(object me,string pass)
{
    ...
    if( fd < -1 ) 
    {
        tell_object(me,"[IM Message]:□□失□，□稍後再□！\n");
        return 1;
    }
    if(users[fd]["obj"] == me)
    {
        ...
        default:
		    ...
		    m = replace_string(m,"account :","account:");
                    if(sscanf(m,":%s!%s PRIVMSG account:%s",name,mail,msg) == 3 )
                    {
                        ...
                        {
                            tell_object(users[fd]["obj"],"\n[IM Message]:"+mail+"想要把你加入他的好友名□，同意的□用 im yes，不同意的□，用 im no。\n");
                        }
...
//                            case "MSN - Logging in: Authenticated, getting buddy list":
                            ...
                            case "No accounts known. Use 'account add' to add one.":
                                tell_object(users[fd]["obj"],"\n[IM Message]:□使用 im register msn 或是 im register yahoo □□□□！\n");
                                break;
                    ...
                    // 上站□息 :paiting!paiting@hotmail.com JOIN :&bitlbee
			...
			CH_D->register_relay_channel("qq");
                    ...
                    // 下站□息 :paiting!paiting@hotmail.com QUIT :Leaving...
                    ...
                    // □□□□上 :root!root@rw.twku.net MODE &bitlbee +v paiting
                    if(sscanf(m,":root!root@rw.twku.net MODE &bitlbee +v %s",id) == 1)
                    {
                        send_who(fd,id);
                        continue;
                    }
                    // □□□away或忙□ :root!root@rw.twku.net MODE &bitlbee -v paiting
                    if(sscanf(m,":root!root@rw.twku.net MODE &bitlbee -v %s",id) == 1)
                    {
                        send_who(fd,id);
                        continue;
...
//    i = efun::socket_write(fd,message);
    switch(i)
    {
        case EEFDRANGE:       TELL("描述值 (descriptor) 超出□□。"); break;
        case EEBADF:          TELL("□效的描述值。"); break;
        case EESECURITY:      TELL("企□□反安全。"); break;
        case EENOADDR:        TELL("socket 未指定位址。"); break;
        case EEBADADDR:       TELL("位址格式的□□。"); break;
        case EENOTCONN:       TELL("socket 尚未□接。"); break;
        case EEALREADY:       TELL("操作已在□行中。"); break;
        case EETYPENOTSUPP:   TELL("不支援此物件型□。"); break;
//        case EEBADDATA:       TELL("送出的□料含有太多巢□□次 (nested level)。"); break;
        case EESENDTO:        TELL("sendto 的□□。"); break;
        case EEMODENOTSUPP:   TELL("不支援此 socket 型□。"); break;
        case EEWOULDBLOCK:    TELL("操作□□停□ (block)。"); break;
        case EESEND:          TELL("送出 (send) 的□□。"); break;
...
void process_who(int fd,string str)
{
    ...
    //:rw.twku.net 352 whatup &bitlbee whatup 218-184-22-55.cm.dynamic.apol.com.tw rw.twku.net whatup H :0 小ｘ□
    if(sscanf(str,":%*s 352 %*s &bitlbee %s %s %*s %s %s :%*d %s",
       id,ip,name,status,nick) == 9) 
    users[fd]["list"][name] = ({ id,ip,name,status,nick});
    //:rw.twku.net 352 whatup whatup_ whatup mail2000.com.tw rw.twku.net whatup_ H :0 □缸-有□要 gmail □□，我有一堆-.
...
int process_send_msg(object me,string who,string msg)
{
    ...
    if(undefinedp(users[fd]) ||
       undefinedp(users[fd]["list"][who]))
        return notify_fail("[IM Message] 即□□息上的好友名□□有□□人！\n");
//	socket_write(fd,"PRIVMSG &bitlbee :set charset 'utf-8'\r\n");	
...
//		socket_write(fd,"PRIVMSG &bitlbee :set charset 'gb2312'\r\n");	
    {
        ...
        if (n[4]&&n[0]&&n[1])
        	tell_object(me,sprintf(HIW HBBLU"你用即□□息□□ %s : %s\n"NOR,
            	      n[4]+"("+n[0]+"@"+n[1]+")\n["+who+"]",msg));
    }
    return 1;
}
...
void auto_login()
{
	object me = this_object();
        ...
        {
//            TELL("[IM Message]:你正在登入中，□用 im quit □□。");
//            return;
