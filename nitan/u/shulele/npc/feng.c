...
// 在啟動拍賣之前，首先請用cshshop指令初始化所有店舖狀態
...
int do_start(string arg)
{
        ...
        } else
        {
                ...
                if (arg)
                {
                        CHANNEL_D->do_channel(ob, "sys", "店舖拍賣系統由" + me->name(1) +
                                                  "成功啟動。運行模式：自動。");
		        ...
		        // 拍賣設置為手工運行
                        ...
                        msg += HIR "您選擇了通過" HIW "人工操作" HIR "的方式來啟"
                               "動拍賣進程。\n\n" NOR;
                        msg += WHT "拍賣的指令為(" HIY "auction <店舖ID>" NOR +
                               WHT ")。\n" NOR;
                        ...
                        message_vision(CYN "$N" CYN "擦了擦汗，賠笑道：這次的拍賣"
                                       "由$n" CYN "親自主持。\n" NOR, ob, me);
                        message_vision(HIC "$N" HIC "幹笑兩聲，將手中的錘子鄭重交"
                                       "給$n" HIC "。\n" NOR, ob, me);
                        operator = me;
	...
	//  非自動的
		...
		// 已經全部拍賣完畢了
                if (sizeof(shop_list) < 1)
                {
                        set("can_finish", 1);
                        save();
                        return notify_fail(WHT "所有店舖都已經拍賣結束了，您現"
...
int stop_auction()
{
        ...
        if (query("wait_jupai"))
        {
                message_vision(BEEP + HIY "\n$N" HIY "舉起手中的小木棰用"
                               "力的一敲，只聽得『當』的一聲巨響。\n" NOR,
                               operator);
                message_vision(HIY "$N" HIY "大聲宣布道：現在" HIC + now_shop +
                               HIY "店舖的所有權拍賣流標了。\n" NOR,
                               operator);
                ...
                } else
                {
                        ...
                        if (sizeof(shop_list) < 1)
                        {
                                set("can_finish", 1);
                                save();
                                tell_object(operator, WHT "所有店舖都已經拍賣結"
        ...
        } else            
        {
                ...
                } else
                {
                        ...
                        message_vision(BEEP + HIY "\n$N" HIY "舉起手中的小木棰用"
                                       "力的一敲，只聽得『當』的一聲巨響。\n" NOR,
                                       operator);
                        message_vision(HIY "$N" HIY "大聲宣布道：成交！恭喜" +
		...
		// 自動拍賣
                ...
                } else
                {
                        ...
                        if (sizeof(shop_list) < 1)
                        {
                                set("can_finish", 1);
                                save();
                                tell_object(operator, WHT "所有店舖都已經拍賣結束了，您現"
                                                      "在可以結束(" HIY "finish" NOR + WHT
                                                      ")拍賣了。\n"NOR);
                                return 1;
                        }
...
int do_jupai(string arg)
{
	...
	//  手工拍賣，檢查執行巫師是否還在，如果不在則進程轉為自動
        ...
        if (! query("now_shop"))
               return notify_fail(operator->name(1) + "還沒有宣布"
                                  "要拍賣哪家店舖，你急個啥？\n");
...
int do_qiao()
{
        ...
        if (m = query("on_jupai"))
        {
                ...
                return notify_fail("您準備繼續宣布拍賣哪個店舖？\n");
...
int finish_auction(string arg)
{
        ...
        message_vision(CYN "\n$N" CYN "滿意的點了點頭，道：本次店舖拍賣圓"
                       "滿結束。\n" NOR, operator);

        CHANNEL_D->do_channel(ob, "sys", "店舖拍賣完畢。" + operator->name(1) +
                                  "結束了拍賣進程！");

        if (operator == ob)
        {
                SHOP_D->open_all(operator);
                message_vision(CYN "$N" CYN "笑道：現在所有拍出店舖全部"
                               "開放，大家忙去吧。\n" NOR, operator);
        } else
        ...
        shout(HIY "【商界傳聞】" NOR + WHT + operator->name() + WHT "[" +
              operator->query("id") + WHT "]：拍賣大會到此結束，各位可用"
              "(" HIY "listshop" NOR + WHT ")查看各家店舖詳情。\n" NOR);
...
int do_autoauction()
{
        ...
        CHANNEL_D->do_channel(ob, "sys", me->query("id") + "將拍賣進"
                                  "程轉入自動模式！");
        set("operator", "feng tiannan");
        save();
