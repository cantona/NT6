...
void create()
{
        set_name(HIY"光之劍"NOR, ({"light jian", "lightsword"}));
        set_weight(15000);
...
message_vision(HIY"$N身上的傷輕了不少!!\n"NOR, me,victim);
}
if (random(4)==1)
{
message_vision(HIY"$N手上的光之劍突然聖光大作!!$N的身後出現了聖火!!\n"NOR, me,victim);
message_vision(HIY"光之禮讚!!  $N全身發出了耀眼的光芒！！\n"NOR, me,victim); 
 if ( random(me->query("combat_exp"))>(int)victim->query("combat_exp")/3)
{
if (!victim->is_busy())
victim->start_busy(2);
message_vision(HIY"$n覺的掙不開眼了!!\n"NOR, me,victim);
}
else message_vision(HIY"$n飛身躍起，逃過了一難。!\n"NOR, me,victim);
}
if (me->is_busy())
{
me->start_busy(1);
message_vision(HIY"$N在光之禮讚的作用下，恢復了行動!!\n"NOR, me,victim);
}
}