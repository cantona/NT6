...
void create()
{
	...
	if( clonep() )
		...
		set("unit", "把");
		set("long",@LONG
這是一把藍色的，很COOL的電吉他，上面繫着幾根弦，彈（tan）
起來發出一陣美妙的韻律。
LONG
);
	set("value", 0);
...
int do_tan(string arg)
{
	...
	if( arg == "jita"  || arg == "吉他"){
	   	message_vision("$N很COOL的拿起一把電吉他，激昂的音樂聲震撼着每一個少女的心懷。\n",me);
	   	me->add("qi",-10);
	   	me->set("title",HIR +"風"+ HIY +"總是"+ HIW +"吹過"NOR);
	switch( random(22) ) {
                case 0:
           	message("channel:chat",HIY"〖"+me->query("name")+"笑傲江湖演唱會〗："HIC"Don~t Break My Heart，再次温柔，喜悦總是出現在我夢中～～\n"NOR,users());
		break;
                case 1:
                message("channel:chat",HIY"〖"+me->query("name")+"笑傲江湖演唱會〗："HIC"我的熱情，好像一把火，燃燒了整個沙漠，太陽見了我，也會躲着我～～\n"NOR,users());
		break;
                case 2:
		message("channel:chat",HIY"〖"+me->query("name")+"笑傲江湖演唱會〗："HIC"我承認都是月亮惹的禍，那樣的夜色太美你太温柔～～\n"NOR,users());
		break;
        	case 4:
		...
		break;
	   	case 9:
		message("channel:chat",HIY"〖"+me->query("name")+"笑傲江湖演唱會〗："HIC"我想哭但是哭不出來，跟着～～\n"NOR,users());
		break;
		case 10:
		message("channel:chat",HIY"〖"+me->query("name")+"笑傲江湖演唱會〗："HIC"説了是無心，做了更無情，都説做無情人有何必再寫信～～\n"NOR,users());
		break;
		case 11:
		...
		break;
		case 14:
		message("channel:chat",HIY"〖"+me->query("name")+"笑傲江湖演唱會〗："HIC"冷雨夜我不想歸家，盼望你背影，可苦笑望雨點，雖知要説清楚，可惜我沒膽試～～\n"NOR,users());
		break;
		case 15:
		message("channel:chat",HIY"〖"+me->query("name")+"笑傲江湖演唱會〗："HIC"我會搽去我不小心滴下的淚水，還會裝做一切都無所謂～～\n"NOR,users());
		break;
		case 16:
		message("channel:chat",HIY"〖"+me->query("name")+"笑傲江湖演唱會〗："HIC"讓我將你心兒摘下，試着讓它漫漫融化，看我在你心中是否仍完美無瑕～～\n"NOR,users());
		break;
		case 17:
		message("channel:chat",HIY"〖"+me->query("name")+"笑傲江湖演唱會〗："HIC"當他對你説一往情深，你千萬不要信以為真，不要輕易的付出你的吻～～\n"NOR,users());
		break;
		case 18:
		...
		break;
		case 21:
		message("channel:chat",HIY"〖"+me->query("name")+"笑傲江湖演唱會〗："HIC"今天晚上的星星很少，不知道它們到哪裏了～～\n"NOR,users());
		break;
		case 22:
		message("channel:chat",HIY"〖"+me->query("name")+"笑傲江湖演唱會〗："HIC"哎喲向着天空拜拜呀勇敢站起來，老天愛笨小孩。\n"NOR,users());
		break;
		}
	return 1;
	}
