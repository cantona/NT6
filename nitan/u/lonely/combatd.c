...
string *body_damage_me_msg = ({
        ...
        append_color(BLU "你只覺得$l處劇痛，連帶着體內真氣一陣外泄。\n" NOR, BLU),
});
...
string *catch_hunt_human_msg = ({
        ...
        append_color(HIW "$N和$n一碰面，二話不説就打了起來！\n" NOR, HIW),
...
string *catch_hunt_bird_msg = ({
        ...
        append_color(HIW "$N一聲鋭鳴，猛然向$n發動攻擊！\n" NOR, HIW),
});
...
string *unarm_parry_throwing = ({
        ...
        append_color(HIR "$n脱下一隻鞋子對準$w一撈，$w掉入$n的鞋中。\n" NOR, HIY),
});
...
string damage_msg(int damage, string type)
{
        ...
        case "點穴":
                ...
                if( damage < 1000 )return "結果$n一聲大叫，$l的穴道被點中,疼痛直入心肺！\n"; else
                                   return "結果只聽見$n一聲慘叫，一陣劇痛夾雜着麻癢遊遍全身，跟着直挺挺的倒了下去！\n";
                break;
        case "抽傷":
        case "鞭傷":
                if( damage < 50 )  return "結果只是在$n的皮肉上碰了碰，好像只蹭破點皮。\n"; else
        ...
        case "挫傷":
                ...
                if( damage < 200 ) return "結果$N這一下砸個正着，$n的$l登時腫了一塊老高！\n"; else
                if( damage < 400 ) return "結果$N這一下砸個正着，$n悶哼一聲顯然吃了不小的虧！\n"; else
        ...
        case "凍傷":
                ...
                if( damage < 1000 )return "結果$p身子打顫，臉色蒼白，嘴脣凍得發紫，牙關格格直響！\n";else
...
varargs string eff_status_msg(int ratio, int dam_type) 
{
        ...
        if( ratio > 60 ) {
                ...
                else    return HIY "身上受傷不輕，正在流着血。" NOR;
        }
        if( ratio > 40 ) {
                if (dam_type == 3)
...
varargs mixed skill_power(object ob, string skill, int usage, int delta)
{
        ...
        case SKILL_USAGE_ATTACK:
                ...
                if( query("jingmai/finish", ob) )
                        buff = ZHOUTIAN_D->query_jingmai_effect("attack"); // 經脈衝穴效果 1640
                else
        ...
        case SKILL_USAGE_DEFENSE:
                ...
                if( query("jingmai/finish", ob) )
                        buff = ZHOUTIAN_D->query_jingmai_effect("defense"); // 經脈衝穴效果 1140
                else
...
varargs int do_attack(object me, object victim, object weapon, int attack_type, int flag)
{
        ...
        if( att = SKILLS_D->skill_query_power(martial_skill) ) {
                ...
                my_temp["fight"] = copy(fight);         // 該技能增加本人命中能力
                fight = ([]);
                fight["dodge"]  = to_int(att[DODGE]);
                fight["parry"]  = to_int(att[PARRY]);
                your_temp["fight"] = copy(fight);       // 該技能降低對手躲閃能力
        } else {
#endif
                ...
                if( random(ap + pp) < pp ) {
                        ...
                        if( attack_skill == "throwing" && your["race"] ==  "人類" ) {
                                        ...
                                        if( weapon2 ) {
                                                ...
                                                if( my["jiali"] > weapon2->weight() ) {
                                                        ...
                                                        if( weapon2->is_item_make() ) {
                                                                parry_msg += HIW "$n" HIW "只覺得虎口劇震，兵器脱手而出！。\n" NOR;
                                                        } else {
                                ...
                                if( random(victim->query_skill(defense_skill, 1)) > me->query_skill("force", 1)/2 ) {
                                        ...
                                        damage+=query_temp("apply/damage2", me);   // 裝備改造傷害
        ...
        //
                        ...
                        if( damage > 0 ) {
                                ...
                                */
                                ...
                                // 發揮勛章--碧海殘钁的作用
                                if( objectp(ob = query_temp("armor/medal", me)) && ob->query_magic_item() == "碧海殘钁" )
                                {
                                        damage_info += HIC "$N" HIC "的碧海殘钁青光一閃，「碧海蒼穹」呈現，發出震耳般的海嘯聲 ……\n" NOR;
                                        damage += damage / 2;
                                        if( !victim->is_busy() )
                                                victim->start_busy(3);
                                }
                         ...
                         int damage, int percent, mixed final)
{
        ...
        */
        ...
        while (damage > 0)
        {
                ...
                if( objectp(ob = query_temp("armor/medal", me)) && ob->query_magic_item() == "碧海殘钁" )
                {
                        msg += HIC "$N" HIC "的碧海殘钁青光一閃，「碧海蒼穹」呈現，發出震耳般的海嘯聲 ……\n" NOR;
                        damage += damage / 2;
                        if( !target->is_busy() )
                                target->start_busy(3);
                }
...
int player_escape(object killer, object ob)
{
        ...
        {
        case 0:
                msg = "突然只聽幽幽一聲長歎，一掌輕輕按來。$N大吃一驚，不及"
        ...
        if( (environment(killer) == environment(ob)) &&
            killer->is_fighting(ob) ) {
                ...
                msg = "正逢一老者路過，見了" + ob->name() + "暈倒在地，歎口"
                      "氣，將他扶起帶走了。\n";
        ...
        */
        ...
        tell_object(ob, "你睜開眼來，看到兩位老者正在靜坐修煉。\n"
                    HIG "龍島主告訴你：" + RANK_D->query_respect(ob) +
                        "，你要想離島不妨和我説一聲(ask long about 離島)。\n" NOR);
        return 1;
}
...
void killer_reward(object killer, object victim)
{
        ...
        while( 1 )
        {
                ...
                if( playerp(victim) ) {
                        ...
                        if( query("pk_score", victim)>0){
                                ...
                                follow_msg = "聽説殺人兇手" + victim->name(1) +
                                             "被" + killer->name(1) + "在" +
                       ...
                       if( playerp(killer) && killer->in_team() && objectp(killer->query_team_leader()) ) {
                        ...
                        if( !killer->query_condition("killer") ) {
                                follow_msg = "聽説官府發下海捕文書，緝拿殺人兇手" +
                                             killer->name(1) + "。";
                                killer->apply_condition("killer", 500);
                        } else {
                                follow_msg = "聽説官府加緊捉拿累犯重案的暴徒" +
                                             killer->name(1) + "。";
                                killer->apply_condition("killer", 800 +
                                        (int)killer->query_condition("killer"));
                        }
        ...
        if( playerp(victim) ) {
                ...
                CHANNEL_D->channel_broadcast("combat",
                        sprintf("聽説%s" HIR + msg + "。", victim->name(1)));
