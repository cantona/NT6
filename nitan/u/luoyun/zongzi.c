...
int do_eat(string arg)
{
	...
	{
		tell_object(me,HIY "端午節使者突然在一陣煙霧中出現，一把抓過你手中的粽子：“你今天已經吃過了吧？可別貪心哦！”説完將粽子收走了。\n"NOR);
	...
	if ( (CHINESE_D->chinese_time(7, ctime(time()))) != "2013/06/12" &&
		...
		tell_object(me,HIY "端午節使者突然在一陣煙霧中出現，一把抓過你手中的粽子：“你這粽子已經過期了吧？”説完將粽子收走了。\n"NOR);
		destruct(this_object());	
		return 1;
	} else {
		tell_object(me, HIC "你看着美味的粽子，想也不想一口吞進了肚裏！\n" NOR);     	
