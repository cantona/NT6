...
int do_launch(string arg)
{
        ...
        if (me->is_busy())
                return notify_fail("什麼事情都等你忙完了再説。\n");
        ...
        else
        {
                ...
                ob->ban_say_until(10 * 60, "你現在被炸得頭暈眼花，説不出話");
...
int do_aim(string arg)
{
        ...
        if (arg && sscanf(arg, "%s with %s", arg, my_id) == 2 && ! id(my_id))
                return notify_fail("這裏沒有(" + my_id + ")這樣東西。\n");
        ...
        if (me->ban_say())
                return notify_fail("你現在説不出話來...導彈也沒法發射。\n");

        tell_object(me, HIY "你掏出一個" + name() + HIY "，東瞄瞄，西瞄瞄。\n" NOR, me);
        message("vision", HIR "【觀察衞星】緊急報告，" + ob->name(1) +
                          HIR "已經被人用" + name() +
                          HIR "瞄準了。\n" NOR, ob);
