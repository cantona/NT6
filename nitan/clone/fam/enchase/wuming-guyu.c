...
void create()
{
        ...
        if (clonep())
                ...
                set("unit", "塊");
                set("long", HIC "這是一塊不知名的古玉，看上去非常古老，不知產于哪一年代。\n" HIG
                                "需要通過鑒定，此玉的真面目才能顯現，鑒定後可隨機出現以下玉石之一：\n" HIB
                                "古玄玉（1級）、古玄玉（2級）、古玄玉（3級）\n" HIW
                                "玄冰石（1級）、玄冰石（2級）、玄冰石（3級）\n" HIR 
                                "鑒定前請確認你的背包中的負重至少剩余5% 。\n" HIC
                                "在揚州魯班處，輸入指令 " + CMD_JD + " 來請求魯班鑒定無名古玉。\n" NOR);
...
int do_jianding(string arg)
{
        ...
        if (! arg || arg != query("id"))
                return notify_fail("你想鑒定什麼？\n");
                
                        
        env = environment(me);
        
        if (base_name(env) != "/d/room/xiaoyuan")
                return notify_fail("這個物品好象只有魯班才能鑒定，你還是上他那去吧。\n");
                
        message_vision(HIY "$N拿出" + name() + HIY "讓魯班鑒定。\n", me);
        ...
        */
        ...
        tell_object(me, HIG "魯班將鑒定後的玉石交給你。\n" NOR);
