...
int do_decide()
{
        ...
        if( query_temp("want_join", me) )
        {
                ...
                if( query_temp("want_tuoli", me) )
                {
                        ...
                        {
                                message_vision(HIR "$N" HIR "將$n" HIR "的" NOR + ob->name(1) + HIR "注銷了。\n" NOR, this_object(), me);
                                destruct(ob);
                        }
                        return 1;
                 }
...
int do_yao()
{
        ...
        if (objectp(present("shang ling", me)))
                return notify_fail(CYN+query("name", ob)+"皺了皺眉道：我的天，你連這都要騙？果然是姦商的料。\n"NOR);
