...
void create()
{
        set_name(HIR "牛氣沖天新年紅包" NOR, ({ "hongbao2009"}) );
        set_weight(100);
        if (clonep())
                set_default_object(__FILE__);
        else {
                set("long", HIR "這一是個新年紅包，上面用金色的字體寫着" HIY "「牛年快樂，牛氣沖天」" HIR " ！\n"
                                "拆開(chai)紅包可以獲得新年壓歲錢及小小的經驗潛能獎勵。並有機會獲得隨機物品一件。\n"
                                "*請於2009年3月1日前使用紅包，過期則使用無效！！！\n\n" NOR);
                set("value", 0);
                set("no_give", 1);
                set("no_sell", 1);
                set("no_store", 1);
...
int do_chai(string arg)
{
        ...
        if (random(10) == 1)
        {
                ...
                if (obgift)
                {
                        ...
                        CHANNEL_D->do_channel(this_object(), "rumor", "聽説" + me->name() + "開啟新年紅包獲得隨"
                                                                    "機物品" + obgift->name() + HIM "獎勵！\n");  
                        obgift->move(me, 1);
                        log_file("hongbao2009", HIY + me->query("id") + "從紅包中獲得 " + obgift->name() + "。\n" NOR);
                }
        }

        me->save();

        log_file("hongbao2009", me->query("id") + " 於 " + ctime(time()) + " 打開紅包。(/clone/misc/hongbao2009) \n");
