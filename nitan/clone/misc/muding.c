...
void create()
{
        ...
        set_weight(1000);
        set("long", @LONG
一只木制的鼎，裡面散發出香料的氣味，但是細細感覺下卻有一股腥味。
LONG );
        set("unit", "只");
        set("value", 2500);
...
int do_make(string arg)
{
        ...
        if (! arg)
                return notify_fail("你想利用" + name() + "制作什麼？\n");
        ...
        if (me->query_skill("poison", 1) < 50)
                return notify_fail("你的基本毒技火候不夠，不能制毒。\n");

        if (me->query_skill("huagong-dafa", 1) < 80)
                return notify_fail("你的化功大法火候不夠，不能制毒。\n");
        ...
        if (! amount) return notify_fail(name() + "又沒啥毒蟲，你怎麼制毒？\n");
