...
void create()    
{
        ...
        if (clonep())   
                ...
                set("long", sort_msg(CYN"楚國的都城已被晉國的兵馬圍困了三年。晉國出兵伐楚，" 
                                     ...
                                     "和幹將兩大劍師聯手所鑄。但是兩位大師卻不這樣認為，他們" 
                                     "說泰阿劍是一把諸侯威道之劍早已存在，只是無形、無跡，但" 
                                     "是劍氣早已存于天地之間，只等待時機凝聚起來，天時、地利" 
                                     "、人和三道歸一，此劍即成。晉國當時因為強大，晉王當然認" 
                                     "為自己最有資格得到這把寶劍，但是事與願違，此劍卻在弱小" 
                                     "的國鑄成，出劍之時，劍身果然天然鐫刻篆體“泰阿”二字，" 
                                     "可見歐冶、幹將所言不虛晉王當然嚥不下這口氣，于是向楚王" 
                                     "索劍，楚王拒絕，于是晉王出兵伐楚，以索劍之名借機滅掉楚" 
                                     ...
                                     "一聲：泰阿劍啊，泰阿劍，我今天將用自己的鮮血來祭你！于" 
                                     "是，拔劍出鞘，引劍直指敵軍。匪夷所思的奇跡出現了：只見" 
                                     "一把磅礡劍氣激射而出，城外霎時飛砂走石遮天蔽日，似有猛" 
                                     "獸咆哮其中，晉國兵馬大亂。片刻之後，旌旗僕地，流血千裡" 
        ...
        set("no_put",1); 
        set("no_steal",1);  
        set("wield_msg",CYN"$N[噌]的一聲抽出一把泰阿劍，只見一把磅礡劍氣激射而出，霎時，飛砂走石遮天蔽日，"
                              "似有猛獸咆哮其中。\n"NOR);
