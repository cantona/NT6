...
int main(object me, string arg)
{
        ...
        if (! mapp(req = query("mixture", get_object(med))))
                return notify_fail(med->name() + "好像無法配制...\n");
        ...
        if( query("no_fight", environment(me)) && 0 )
                return notify_fail("你在這裡制藥只怕要打擾到別人。\n");

        qm=query_temp("handing", me);
        if (! qm)
                return notify_fail("你的先把能夠磨藥的研砵拿(hand)在手上才行。\n");

        if (! qm->can_make_medicine())
                return notify_fail(qm->name() + "好像無法發揮研砵的作用吧。\n");
        ...
        msg += "，然後小心翼翼的把它們放到" + qm->name() + "裡面，開始制藥。\n";
        msg = sort_string(msg, 64);
        write(msg);
...
這個指令可以讓你煉制某樣藥品，如果沒有指明藥品則會列出當前
你會煉制的藥品。如果在指明的藥品後面添加一個"?" 則會列出煉
制這種藥品需要的材料。
