...
mapping *action = ({ 
  ...
  ([      "action" : "$N猛地門户大開，迎着$n的掌勢合身而上，掌勁及身的一霎那，$N突然一個鷂子翻身，電光火石間$n的掌力已然全部返回，正是少林不傳之密「乾坤逆轉」", 
    "force" : 480, 
    "dodge" : 20, 
    "parry" : 10, 
    "damage": 50, 
...
{ 
    if( query_temp("weapon", me) || query_temp("secondary_weapon", me) )
        return notify_fail("練袖裏乾坤須空手。\n"); 
    if ((int)me->query_skill("shaolin-xinfa", 1) < 100) 
        return notify_fail("你的少林心法不夠，無法學習袖裏乾坤。\n"); 
    if( query("max_neili", me)<1500 )
        return notify_fail("你的內力太弱，無法練習袖裏乾坤。\n"); 
    return 1; 
} 
...
int practice_skill(object me) 
{
    ...
    if( query("neili", me)<20 )
        return notify_fail("你的內力不夠練習袖裏乾坤。\n"); 
