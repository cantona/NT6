...
int valid_learn(object me)
{
        if( query("int", me)<24 && me->query_int()<32 )
                return notify_fail("琴棋書畫乃是極其風雅之事，" + 
                       RANK_D->query_respect(me) + "似乎太過于俗氣了些。\n");
...
int do_play(string arg)
{
            ...
            ((arg == "music") || (arg == "zheng") || (arg == "gu zheng")))
        {
                ...
                {
                        message_vision("$N擺出一張古箏，錚錚琮琮彈了幾聲，只聽得曲調平和，洋洋洒洒，頗有佳意。\n", me);
                        return 1;
                }
                me->receive_damage("jing", 10);
                me->receive_damage("qi", 10);
                me->improve_skill("art", (int)me->query_int() / 4 + (int)me->query_skill("art", 1) / 10);
                tell_object(me, "你擺出一張古箏，調了調弦，彈了起來，你只覺得連日來煩燥的心情好了許多。\n");
        ...
        if (present("wei qi", this_player()) && ((arg == "wei qi") || (arg == "qi") || (arg == "go")))
        {
                ...
                if (me->query_skill("art", 1) > 100)
                {
                        message_vision("$N擺出一張棋盤，只覺棋面布局盡在籌幄只之中，無密可研。\n", me);
                        return 1;
                }
                ...
                me->receive_damage("qi", 10);
                me->improve_skill("art", (int)me->query_int() / 4 + (int)me->query_skill("art", 1) / 10);
                tell_object(me, "你擺出一張棋盤，研究起圍棋來，只覺得心平氣和，很快就仿佛入定了。\n");

                message_vision("只見"+query("name", me)+
                               "雅興大發，竟打起棋譜來，對外界仿佛不聞不問，一副莫測高深的模樣。\n",
                               environment(me), ({me}));
