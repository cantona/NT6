...
int perform(object me, object target)
{
      int i, jiali, count;
       ...
       if( !target
        ...
        || !living(target))
                return notify_fail("「修羅燄」攻擊只能對戰鬥中的對手使用。\n");
      ...
      if( !objectp(weapon=query_temp("weapon", me)) || 
        ...
        if (me->query_skill_mapped("blade") != "jiechen-dao")
                return notify_fail("你必須使用戒塵刀來施展「修羅燄」。\n");

        if(me->query_skill("jiechen-dao", 1) < 180 )
                return notify_fail("你的戒塵刀火候還嫌不夠，這「修羅燄」絕技不用也罷。\n");

        if(me->query_skill("blade", 1) < 180 )
                return notify_fail("你的基本刀法還不夠嫻熟，使不出「修羅燄」絕技。\n");

        if( (int)me->query_skill("hunyuan-yiqi", 1) < 140 )
                return notify_fail("你的心意氣混元功等級不夠，使不出「修羅燄」絕技。\n");

        if( (int)me->query_con() < 34)
                return notify_fail("你的身體還不夠強壯，強使「修羅燄」絕技是引火自焚！\n");

        if ( me->query_skill_mapped("force") != "hunyuan-yiqi")
           return notify_fail("你現在這內功平平無奇，如何使得出「修羅燄」絕技來！？\n"); 

        if( query("max_neili", me)<3000 )
           return notify_fail("你的內力修為不夠，這「修羅燄」絕技不用也罷。\n");

        if( query("neili", me)<1000 )
           return notify_fail("以你目前的內力來看，這「修羅燄」絕技不用也罷。\n");
