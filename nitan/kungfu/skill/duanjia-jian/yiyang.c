...
int perform(object me, object target)
{
        ...
        if( query_temp("yiyang", me) )
                return notify_fail("你已運一陽指指力于武器上了！\n");
        ...
        if (me->query_skill("force",1)<100
                ...
                return notify_fail("你的功力不夠，無法運用一陽指指力于武器上！\n");

        if( !objectp(weapon=query_temp("weapon", me)) || query("skill_type", weapon) != "sword" )
                return notify_fail("你沒有使用正確的武器，無法運一陽指力于其上。\n");
        ...
        if( query("weapon_prop/damage", weapon) >= 5000 )
                return notify_fail("你發現這件武器打造的精致無比，巧奪天功，無法灌注內力于其上！\n");
...
void checking(object me, object weapon)
{
        ...
        if( query_temp("yiyang", me)){
           if (!weapon)
           {
                ...
                remove_effect(me, weapon);
                tell_object(me, HIY "\n你的"+weapon->name()+"已毀，不得不收回灌注于其上的一陽指指力。\n\n" NOR);
                return;
           }
