...
int exert(object me, object target)
{
        ...
        for(i=0; i<sizeof(inv); i++)
                if( query("equipped", inv[i] )
                        ...
                        i = sizeof(inv)+1;
        if (i==sizeof(inv)+2)
        return notify_fail("你的伙伴看來還不打算練功。\n");
    ...
    if ( k >=2 ) {
        ...
        if( query("jing", target)<query("max_jing", target)*90/100 )
                return notify_fail("你的伙伴現在精不夠，無法控制內息的流動！\n");
        ...
        if( query("jingli", target)<query("max_jingli", target)*60/100 )
                return notify_fail("你的伙伴現在精力不夠，無法控制內息的流動！\n");
        ...
        if( query("qi", target)<query("max_qi", target)*60/100 )
                return notify_fail("你的伙伴現在的氣太少了，無法產生內息運行全身經脈。\n");
        ...
        if( query_temp("pending/xiuxi", target) != me )
        {
                ...
                write(YEL "由于對方是由玩家控制的人物，你必須等對方同意才能共同修習玉女心經。\n" NOR);
                return 1;
        }
        else
        {
...
int xiuxi(object me)
{
        ...
        int xj_lvl_target = (int)target->query_skill("yunu-xinjing",1);        //伙伴心經級別
        object *inv;
        int i;
