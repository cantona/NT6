...
void pu_hit(object me, object target)
{
        ...
        else if( random(ap+pp)<pp && random(5) == 0 && query("race", target) == "人類" )
        {
                ...
                if(weapon)
                {
                        ...
                        target->reset_action();
                        message_vision(HIR"$N猛覺得勁風罩來，心知不妙，手中"NOR + weapon->name() + HIR"脫手飛出，趕緊也是雙掌向前平推。這一下是以硬接硬，剎那之間，兩下裡竟然凝住不動。\n"NOR, target );
                }
                else message_vision(HIR"$N猛覺勁風罩來，心知不妙，也是雙掌向前平推。這一下是以硬接硬，剎那之間，兩下裡竟然凝住不動。\n\n"NOR, target );
