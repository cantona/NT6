...
int party_quest_book(object me)
{
  ...
  if (!me->query("family/family_name"))
   {
                message_vision("$N對著$n說：這位"+RANK_D->query_respect(me)+"何不加入我"+master->query("family/family_name")+"？\n", master, me);
                return 1;
           }
        if (me->query("family/family_name") !=
                master->query("family/family_name"))
        {
                message_vision("$N對著$n說：我不是你的掌門人，你回你們" + me->query("family/family_name") + "去吧。\n", master, me);
                return 1;
        }
        ...
        {
                message_vision(WHT"$N對著$n說：你學藝未精，不宜下山。\n"NOR, master, me);
                return 1;
        }
        ...
        if ( me->query("quest/book") && !me->query( "quest/book/finished" ) )
        {
                ...
                {
                        message_vision("$N對著$n說：我不是叫你去取" + me->query("quest/book/book") + "，怎麼空手回來了！？\n", master, me);
                        return 1;
                }
                else
                {
                        message_vision( CYN"$N對著$n說：你跑到哪裡閒逛去了？這次可要小心在意些！\n"CYN, master, me );
    ...
    dir[i]=="yanziwu" ||
                ...
                dir[i]=="npc")
        {
                message_vision("$N對著$n說：本門秘籍？不是好好的麼？\n", master, me);
                return 1;
        }
        ...
        {
                message_vision("$N對著$n說：等等吧，等我回頭清點一下看看。\n", master, me);
                return 1;
        }
        ...
        message_vision("$N對著$n說：本門秘籍流落江湖，聽說最近在" + to_chinese(dir[i]) + "一帶出現。你去把它取回來吧？\n", master, me);
...
int accept_quest_book(object me, object obj)
{
        ...
        {
                message_vision("$N對著$n說：這不是我要你找的書。\n",master,me);
                return 1;
        }
        if ( !me->query("quest/book") || me->query( "quest/book/finished" ) )
        {
                message_vision("$N對著$n說：我沒要這書，你去哪裡拿來的！\n", master, me);
                destruct(obj);
                return 1;
        }
        if (time() - (int)me->query("quest/book/time") > 1500 )
        {
                message_vision("$N對著$n說：你平時練功勤奮點，就不必去這麼久了！下次努力點！\n", master, me);
                destruct(obj);
                quest_failed( me, "book" );
                return 1;
        }
        ...
        message_vision("$N對著$n說：本門秘籍？不錯，你為本門立了一大功！\n", master, me);
  ...
  if (!me->query("family/family_name"))
   {
                ...
                !me->query( "quest/betrayer/finished" ) )
        {
                ...
                {
                        message_vision("$N對著$n說：我不是叫你去殺" + me->query("quest/betrayer/name") + "，怎麼空手回來了！？\n", master, me);
                        return 1;
                }
                else
                {
                        message_vision( WHT"$N對著$n說：你跑到那裡閒逛去了？\n"NOR, master, me );
...
//先選地方 減少系統開支
    ...
    dir[i]=="yanziwu" ||
                ...
                dir[i]=="npc")
        {
                message_vision("$N對著$n說：等等吧，等下山弟子回山後你再來看看。\n", master, me);
                return 1;
        }
        if(!( room = find_object("/d/"+dir[i]+"/"+file[j]) ))
                room = load_object("/d/"+dir[i]+"/"+file[j]);
        ...
        || room->query("no_quest")                //add by qingyun
                || room->query("no_fight"))
        {
                message_vision("$N對著$n說：等等吧，等下山弟子回山再看看。\n", master, me);
                return 1;
        }
  ...
  obj = create_betrayer(me);
        ...
        message_vision("$N對著$n說：本門歸山弟子回報，本門叛徒" + obj->query("name") + "在" + to_chinese(dir[i]) + "一帶出沒。你敢去把這叛徒的人頭給我取回來麼？\n", master, me);
...
int accept_quest_betrayer(object me, object obj)
{
        ...
        {
                message_vision(HIR "$N對著$n說：我叫你殺叛徒，你卻去殺玩家！去死吧你！\n"NOR, master, me);
                destruct(obj);
                quest_failed( me, "betrayer" );
        ...
        {
                message_vision( HIR"$N對著$n說：我叫你殺叛徒，你卻濫殺無辜，蒙蔽師長！去死吧你！\n"NOR, master, me);
                destruct(obj);
...
//        p_bonus = (int)random(e_bonus/4)+35;
    ...
    me->add("family/gongxian",1);
        ...
        message_vision("$N對著$n說：叛徒的人頭？不錯，你為本門清理了門戶！\n", master, me);
        ...
        if ((int)me->query_temp("mebetrayernum") == 10)
        {
                ...
                message_vision(HIW "$N對著$n笑道：你在外遊歷期間，為師閉關參悟，近日出關，武學修為又有所突破！\n", master, me);
...
int cancle_quest_betrayer( object me )
{
        ...
        message_vision( "$N對$n說道：那你就幫我找找秘籍吧。\n", master, me );
        return 1;
}
/////叛徒結束//////////////
  ...
  if (!me->query("family/family_name"))
   {
        ...
        {
                message_vision(WHT"$N對著$n說：你學藝未精，無能鋤姦。\n"NOR, master, me);
                return 1;
        }
        ...
        if ( me->query("quest/thief") && !me->query( "quest/thief/finished" ) )
        {
                ...
                {
                        message_vision("$N對著$n說：怎麼樣，姦細"+ me->query("quest/thief/name")+"的人頭呢！？\n", master, me);
                        return 1;
                }
                else
                {
...
// 為了照顧南少林(shaolin是nanshaolin字符串的子集)，所以從後讀起
  ...
  !obj->move(room))
   {
           if (objectp(obj)) destruct(obj);
           message_vision("$N對著$n說：我這裡有點事要忙，等會兒我再看看吧。\n", master, me);
    return 1;
   }
        ...
        message_vision("$N對著$n說：本門弟子回報，我派周圍有陌生人出現。你去查看一下是不是別派的姦細？\n", master, me);
...
int accept_quest_thief(object me, object obj)
{
        ...
        if (obj->query("userp") == 1)
        {
                message_vision( HIR"$N對著$n說：我叫你殺姦細，你卻去殺玩家！去死吧你！\n"NOR, master, me);
                destruct(obj);
                quest_failed( me, "thief" );
                me->delete_temp("methiefnum");
                master->kill_ob(me);
        ...
        {
                message_vision( HIR"$N對著$n說：我叫你殺姦細，你卻濫殺無辜，蒙蔽師長！去死吧你！\n"NOR, master, me);
                destruct(obj);
                quest_failed( me, "thief" );
                me->delete_temp("methiefnum");
                master->kill_ob(me);
  ...
  e_bonus -= e_bonus/3;
        ...
        me->add("family/fealty", s_bonus);
        me->add("family/gongxian",1);
        me->set_temp("prize_reason","姦細");
        me->set_temp("can_give_prize",1);
        me->set_temp("prize_exp",e_bonus);
        me->set_temp("prize_pot",p_bonus);

        message_vision("$N對著$n說：姦細的人頭？不錯，你為本門立一大功！\n", master, me);
        ...
        if ((int)me->query_temp("methiefnum") == 13)
        {
                ...
                me->add("potential",p_bonus);
                me->add("score", 4000);
                message_vision(HIW "$N對著$n笑道：哈哈哈！真乃可塑之才也，為本門屢立奇功呀！\n"NOR, master, me);
...
int cancle_quest_thief( object me )
{
        ...
        message_vision( "$N對著$n說：還是先把本門叛徒鏟除幹凈再管別的事吧。\n", master, me );
        me->delete_temp("methiefnum");
...
int party_quest_kill(object me)
{
  ...
  if (!me->query("family/family_name"))
  {
        ...
        {
                        message_vision("$N對著$n說：我不是叫你在"HIY+CHINESE_D->chinese_monthday(((me->query("quest/kill/time")-1270000000)*365))+NOR"之前殺了" + me->query("quest/kill/name") + "，現在是"HIY+CHINESE_D->chinese_monthday(((time()-1270000000)*365))+NOR"，我要的人頭呢！？\n", master, me);
                        return 1;
        }
...
int accept_quest_kill(object me, object obj)
{
        ...
        if (obj->query("userp") == 1)
        {
                message_vision( HIR"$N對著$n說：我叫你殺人，你卻去殺玩家！去死吧你！\n"NOR, master, me);
                destruct(obj);
//                quest_failed( me, "thief" );
        ...
        {
                message_vision( HIR"$N對著$n說：我叫你殺人，你卻濫殺無辜，蒙蔽師長！去死吧你！\n"NOR, master, me);
                destruct(obj);
...
//        destruct(obj);
                ...
                {
                        message_vision(CYN "$N" CYN "捂著鼻子看了看" + obj->name()
                ...
                if (quest_count == 100)
                {
                        msg += CYN "$N" CYN "哈哈大笑兩聲道：真有你的，連著上"
                               "百次任務都完成的漂漂亮亮，很好，很好。\n" NOR;
                        special = 1;
                } else
                if (quest_count == 200)
                ...
                if (quest_count == 500)
                {
                        msg += CYN "$N" CYN "嘆道：真是長江後浪推前浪，想不到"
                               "你接連五百次任務都無一疏漏，真是不錯！\n" NOR;
                        special = 1;
                } else
                if (quest_count == 600)
                ...
                if (quest_count == 900)
                {
                        msg += CYN "$N" CYN "深深地嘆道：想不到你接連九百次任"
                               "務都無一疏漏，不易之極！不易之極！\n" NOR;
                        special = 1;
                } else
                if (quest_count == 1000)
                {
                        msg += CYN "$N" CYN "深深嘆了口氣，道：想不到你連著一"
                               "千次任務無一失手，看來我們" +
...
//                if ((quest_count >= 100 && random(200) == 1) || special)
        ...
        if (type==1 || (exp==0 &&pot==0 && score==0))
          return 1;
        message("vision", sprintf(HIC "通過這次鍛煉你獲得了%s點經驗，"
                                  "%s點潛能。\n同時還增"
                                  "長了%s點江湖閱歷，及%s點門"
                                  "派忠誠度。\n" NOR,
                                  chinese_number(exp),
...
int betray(object ob, int depth)
{
        ...
        {
                message_vision("$N冷笑道：“就憑你這點微末道行，也想到江湖上去闖盪，還不叫人笑掉大牙？”\n", master, ob);
                return 1;
        }

        if (depth == 0)
        {
                message_vision(HIR "\n$N眉頭一皺，緊緊盯著$n，目光如箭，$n不禁打了個寒戰。\n"NOR, master, ob);
                ob->start_busy(10);
                call_out("betray", 5, ob, depth + 1);
        }
        else
        {
                message_vision(HIR "\n半晌，$N嘆道：“既然你執意離去，我也不便強留。只是你的一身武功，哪裡來就還哪裡去吧。”\n"NOR, master, ob);
  ...
  if (!me->query("family/family_name"))
   {
        ...
        {
                message_vision("$N對著$n說：你還不是掌門大弟子，擔不起如此重任。\n", master, me);
                return 1;
        }
        ...
        if (objectp(first = NPC_D->create_first(first_dir)))
        {
                ...
                {
                        message_vision("$N對著$n說：你不是掌門大弟子，擔不起如此重任。\n", master, me);
                        return 1;
                }
        }
        ...
        {
                message_vision("$N對著$n說：你不是已經拿了令牌了嗎，還不速速起程。\n", master, me);
                return 1;
        }
        return 1;
}
...
int do_qiecuo(string arg)
{
        ...
        {
                message_vision("$N對著$n說：你忙完了再來吧！\n", master, me);
                return 1;
        }
        if (master->is_fighting())
        {
                message_vision("$N對著$n說：我現在正忙著呢！\n", master, me);
                return 1;
        }
      //family_can_qiecuo作為師父獎勵和特殊perform切磋的區別
              if( !me->query_temp("family_can_qiecuo"))
              {
                ...
                if (fealty_b<learn_b)
                {
                        message_vision("$N對著$n說：你還是多為本門做點貢獻再來吧！\n",master,me);
                        return 1;
                }
                ...
                if( stringp(pfskill = me->query_skill_mapped(pfmartial)))
                {
                        ...
                        {
                                message_vision("$N對著$n說：這個武功我沒練過，你去找其他人吧。\n", master, me);
                                return 1;
                        }
                        ...
                        for ( j = 0; j < sizeof( perform_actions ); j++ )
                        {
                                ...
                                if ( perform_actions[j] == pfarg )
                                {
                                        ...
                                        if(sl<10 || bl<10)
                                        {
                                                message_vision("$N對著$n說：你先打好基礎，再來切磋吧！\n", master, me);
                                                return 1;
                                        }
                                        else if(me->query(pfmsg,1)==1)
                                        {
                                                message_vision("$N對著$n說：你已經領會了這招，還要切磋麼？\n", master, me);
                                                return 1;
                                        }
                                        else
                                        {
        ...
        else
        {
                ...
                {
                        message_vision("$N對著$n說：你只能從基本技能中請教特殊技能。\n", master, me);
                             return 1;
                }
                  if(arg=="force")
                {
                        message_vision("$N對著$n說：內功修為，不是靠切磋能提高的。\n", master, me);
                             return 1;
                }
        ...
        ||(special=="hamagong" && arg=="strike")
                ...
                {
                        message_vision("$N對著$n說：此項技能無法通過切磋來提高！\n", master, me);
                             return 1;
                }
                  else
                {
