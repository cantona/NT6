...
#ifdef EPOLL
static char *get_user_command (int);
#else
...
long inet_out_packets = 0;
long inet_out_volume = 0;
long inet_in_packets = 0;
long inet_in_volume = 0;
#ifdef PACKAGE_SOCKETS
long inet_socket_in_packets = 0;
long inet_socket_in_volume = 0;
long inet_socket_out_packets = 0;
long inet_socket_out_volume = 0;
#endif
#endif
long inet_packets = 0;
long inet_volume = 0;
...
#ifdef EPOLL
	/* delete epoll fd */
	epoll_ctl_del(external_port[i].fd);
#endif
...
void add_message (object_t * who, const char * data, int len)
{
    ...
    if( ip->trans ) //added by Lonely@NT
...
static int send_mssp_val(mapping_t *map, mapping_node_t *el, void *obp){
	...
	if(el->values[0].type == T_STRING && el->values[1].type == T_STRING){
		...
		int len = sprintf((char *)buf, (char *)telnet_mssp_value, el->values[0].u.string, el->values[1].u.string);
		...
		int len = sprintf((char *)buf, (char *)telnet_mssp_value, el->values[0].u.string, el->values[1].u.arr->item[0].u.string);
		add_binary_message(ob, buf, len);
		...
		for(i=1; i < ar->size; i++){
			if(ar->item[i].type == T_STRING){
				add_binary_message(ob, (unsigned char *)&val, 1);
				add_binary_message(ob, (unsigned char *)ar->item[i].u.string, strlen(ar->item[i].u.string));
			}
		}
            ...
            case TS_DO:
                switch ((unsigned char)from[i]) {
                    ...
                    case TELOPT_MSSP:
                    {
                    	...
                    	if(!tmp){
                    		unsigned char buf[1024];
                    		int len = sprintf((char *)buf, (char *)telnet_mssp_value, "NAME", MUD_NAME);
                    	...
                    	if(!tmp){
                    		...
                    		int len = sprintf((char *)buf, (char *)telnet_mssp_value, "PLAYERS", num);
                    		add_binary_message(ip->ob, buf, len);
                    	}
                    	...
                    	if(!tmp){
                    		...
                    		sprintf(num, "%ld", (long)boot_time);
                    		int len = sprintf((char *)buf, (char *)telnet_mssp_value, "UPTIME", num);
                    		add_binary_message(ip->ob, buf, len);
                    	}
                    	//now send the rest
 ...
 * return epoll status
 */
void epoll_status(char *msg)
{
	sprintf(msg,"Now epoll ctl is %d \n"
				"Max epoll ctl is %d \n"
				"all_users[]'s size: %d \n"
				"lpc_sockets[],s size: %d \n"
				,epoll_nb
				,epoll_max
				,max_users
				,max_lpc_socks);
 ...
 * add epoll list
 */
INLINE int epoll_ctl_add(int fd , uint32_t events , uint32_t u32)
{
 ...
 * delete epoll list
 */
INLINE int epoll_ctl_del(int fd)
{
	struct epoll_event ev;
        if (epoll_ctl(epoll_fd, EPOLL_CTL_DEL, fd, &ev) < 0)
	{
		fprintf(stderr, "socket '%d' delete from epoll !fail%s\n",
			  fd, strerror(errno));
		return -1;
	}
	epoll_nb--;
 ...
 * init epoll
 */
INLINE void init_epoll()
{
	int i;
    struct rlimit rt;
	/* reset system open file size */
    rt.rlim_max = rt.rlim_cur = EP_MAX_CTL;
    if (setrlimit(RLIMIT_NOFILE, &rt) == -1)
    {
        perror("\nset rlimit");
        debug_message("Epoll need reset system open file size limit.\n");
        debug_message("Please use (sudo ./driver config.cfg)\n\n");
        exit(1);
    }
    else
    {
	...
	if (-1 == epoll_fd) {
		debug_message("epoll_create: %m.");
		exit(1);
	}
	/*
	 * set new user accept fd in epoll.
	 */
	for (i = 0; i < 5; i++) {
		if (!external_port[i].port)
			continue;
		epoll_ctl_add(external_port[i].fd , EPOLLIN , (uint32_t)(EXTERNAL_PORT | i));
	}
	/*
	 * if addr_server_fd is set, set its fd in epoll.
	 */
	if (addr_server_fd >= 0) {
		epoll_ctl_add(addr_server_fd , EPOLLIN , (uint32_t)ADDR_SERVER_FD);
	}
	return;
}
/*
 * flush lpc socket message
 */
INLINE void flush_lpc_socket(){
...
/*
 * flush all user message
 */
INLINE void flush_user_message(){
	int i;
 ...
 * Process I/O.
 */
INLINE void process_io(struct epoll_event *event)
{
    int i = (uint32_t)(event->data.u32) & SPLIT_MARK;
    /*
	* check for new user connection.
	*/
	if ((uint32_t)event->data.u32 & EXTERNAL_PORT)
	{
     ...
     */
	if((uint32_t)event->data.u32 & USERS_FD)
	{
		if (!all_users[i] || (all_users[i]->iflags & (CLOSING | CMD_IN_BUF)))
			return;
		...
		if (event->events & EPOLLIN)
		{
			...
			if (!all_users[i])
				return;
			/* handle command */
			process_user_command(i);
			/* process_user_command() will flush message */
			return;
		}
    }
#if defined(PACKAGE_SOCKETS) || defined(PACKAGE_EXTERNAL)
     ...
     */
    if((uint32_t)event->data.u32 & PACKAGE_SOCKETS_FD)
    {
		if (lpc_socks[i].state != STATE_CLOSED)
			if (event->events & EPOLLIN)
     ...
     */
    if (addr_server_fd >= 0 && (uint32_t)event->data.u32 & ADDR_SERVER_FD)
    {
		if (event->events & EPOLLIN)
		{
...
static void new_user_handler (int which)
{
	...
	/* check open file size */
#ifdef EPOLL
	/* Reserved 10 size for other program */
	if(new_socket_fd >= EP_MAX_CTL-10)
	{
		debug_message("new_user_handler: accept:Open file size reach %d\n",new_socket_fd);
...
*/
    ...
    logon(ob);
#ifdef EPOLL
    epoll_ctl_add(new_socket_fd , EPOLLIN , (uint32_t)(USERS_FD | i));
#endif    
 ...
 */
#ifdef EPOLL
static char *get_user_command(int i)
{
	interactive_t *ip;
	char *user_command = 0;
 ...
 */
#ifdef EPOLL
void process_user_command(int i)
#else
int process_user_command()
#endif
{
    char *user_command;
    interactive_t *ip = 0;//for if(ip) below
     ...
     */
#ifdef EPOLL
    while(user_command = get_user_command(i)){
#else
